...
Use local Codex and Claude remotely from WeChat or Feishu while keeping real workspace and session context, live progress, approvals, and results. Codex always has one authoritative Host. In the complete multi-frontend mode, Codex App, the controlled CLI, WeChat, and Feishu all connect to one verified official session service and continue the same thread.

> Official builds and uploads are limited to **macOS Apple Silicon (darwin/arm64)** and **Linux amd64**. Other platforms do not receive official binaries or self-update support.
...
- **Share the original task**: the macOS App adapter preserves official tools and interfaces while messaging clients and the official CLI connect to the same session service.
...
# On Apple Silicon macOS or Debian amd64, use Gitee for the same installer and verified mirror assets if GitHub is unreachable
...
On macOS, `codex_host_mode: auto` selects `shared` when App reuse is enabled and Codex App includes the official Node runtime. App, the official CLI, Feishu, and WeChat connect to the same official app-server. The adapter preserves App launch settings, tools, and permissions. Other environments retain the official-daemon or managed compatibility paths. Explicit `daemon` and `managed` modes do not silently switch to `shared`.

Set `"codex_multi_frontend": true` to prepare and verify the shared service before messaging platforms start. For the standard native ACP `app-server` topology (`codex_host_mode` `auto`/`daemon`, no custom socket or `run_as_user`, and App reuse not explicitly disabled), omitting the field is normalized to `true`; an explicit `false` disables this mandatory gate and prevents normalization from enabling an unset App-reuse option. This setting no longer forces Host mode to `daemon`: macOS `auto` may select the App adapter, while other platforms use the official daemon. Missing standalone binaries, conflicting Hosts, or unverifiable identity block startup without opening messaging first or falling back to managed. This setting cannot be combined with `managed`, a custom `app_server_socket`, `run_as_user`, or `codex_app_reuse_daemon: false`.

Except for explicit `--force`, shared managed Hosts, the official daemon, and controlled `weclaw codex cli` sessions run a read-only multi-Host preflight before startup, attachment, or coordinated shutdown. On macOS, candidate executable paths and argv are read with `kern.procargs2`; on Linux, they come from `/proc/<pid>/exe` and `/proc/<pid>/cmdline`. The scan groups the Node wrapper and native child by PGID. Extra Hosts, unreadable process data, or unverifiable authority fail closed. `codex --remote`, help, daemon, proxy, schema-generation tooling, and an App-bundled Code Mode helper carrying the exact `features.code_mode_host=true` marker are not extra shared Hosts; a non-App executable cannot self-exempt with that marker. Read-only preflight stops no process. No path kills an existing App or unknown process by name.

To enable `shared` initially, fully quit the old App and run `weclaw codex app`. WeClaw configures the protected entrypoint for future App launches and verifies `CODEX_HOME` and `CODEX_SQLITE_HOME`; explicit conflicting paths are not overwritten. Subsequent App launches use that entrypoint. Releasing a session does not require restarting App or waiting for background unloading. App relaunch refreshes its tool connection; other launch-setting changes require a coordinated shared-service restart after tasks finish. Switching Host mode restores only launch settings still matching the values recorded by WeClaw. App bundles, signatures, history databases, and writer-lock files remain unchanged.

Native Chrome control can reuse Codex App's Computer Use. Keep App running with native computer use enabled and the required system permissions granted; tasks access Chrome through `cua_repl` and `cua.getApp("Google Chrome")`. The shared Host retains App's plugin configuration and official runtime, and the native tool's authentication connection also reuses the existing Host. This native path can work with API-key authentication; availability after attachment must still be verified in a shared task.

The Browser/Chrome plugins' dedicated browser connection is a separate path and may return `unsupported Codex auth method: apikey`. That error does not mean Computer Use control of Chrome also requires ChatGPT OAuth. The legacy Chrome plugin's exact `node_repl/js` dynamic tool still runs in App while WeClaw waits for its result. `cua_repl` is an MCP tool loaded by the shared Host, not a dynamic tool forwarded through the allowlist. WeClaw never connects directly to the Chrome native pipe or changes upstream authentication or system authorization.

Each shared-mode input reads authoritative state for the original thread: an active turn receives the supplemental input with its current `turnId`, while an idle thread starts the next turn. App, the official CLI, Feishu, and WeChat may select the same thread concurrently; upstream request acceptance determines ordering.

A session binding represents only the thread selected by the current frontend window; it is not writer ownership. `/cx switch` and `/cx new` first prove that the default shared Host is reachable and validate the target with a lightweight `thread/read`. An unreadable Host or target preserves the previous binding; a readable target is committed immediately. The v14 `preparing/ready` fields remain as follower-observer synchronization state and for rollback compatibility, but no longer authorize regular messages. History replay, progress-card, or exact-turn observer failures mark progress synchronization as “syncing” or “degraded” without rolling back a valid binding.

Multiple bound Feishu or WeChat routes may observe and write the same thread. Each route independently receives progress, approval or question displays, and one terminal result. A single-decision broker keyed by `(thread, turn, request)` accepts only one approval or structured-answer submission; every other display converges to externally resolved. On restart, WeClaw restores Host authority, history and pending interactions, observer readiness, and finally terminal-outbox delivery. Observer recovery affects synchronization and terminal catch-up, not whether a new input may use an available runtime. A durable follower also records the exact `allowed_users` identity authorized for that bot, so revocation clears only that route and blocks its guarded pending results. An idle binding performs only lightweight reads and does not use `thread/resume` to retain a long-lived subscription. Once an active turn is confirmed terminal, WeClaw sends `thread/unsubscribe` once while preserving the durable binding, to reduce idle subscriptions; local availability depends on all clients sharing the same service. `/cx release` stops observation and interaction delivery only after the durable release tombstone is saved; when no other frontend or active task needs observation, it sends `thread/unsubscribe` only on the current app-server connection. Release never interrupts the active turn, restarts the Host, or affects another route.
...
Model, reasoning effort, and service tier changes use the official `thread/settings/update` API and apply to subsequent turns. If the original thread is not loaded, WeClaw resumes only that thread, retries the settings request known not to have been submitted, and releases its temporary subscription. It neither creates a replacement session nor reports failure as success.

Approvals and structured questions on the same shared service are displayed to every synchronized observer, but the broker accepts only one decision. When Codex App or another route resolves the request first, the daemon's `serverRequest/resolved` notification idempotently closes the other displays without a default denial or duplicate response. Every regular input re-reads the authoritative thread state: active uses `turn/steer` with `expectedTurnId`, idle uses `turn/start`, and a not-loaded thread is resumed only immediately before writing. Deterministic start/steer races re-read state and submit that input once. A genuinely unavailable Host rejects immediately; WeClaw neither queues nor stores the input for delayed execution.

`shared` connects directly to the same service as App and does not transfer writer ownership through Desktop IPC. Explicit `daemon` retains the older targeted Desktop-follower compatibility path, requiring an active-writer error plus matching thread/turn IPC evidence; this does not establish complete sharing for every App version. `/cx status` separates binding, runtime channel, and progress synchronization. Degraded synchronization does not make a session unwritable.

If the connection disconnects or times out after `turn/start` or `turn/steer` was written, WeClaw compares recent turn/items against the pre-send last-turn baseline and message digest. Authoritative evidence of acceptance resumes observation; no evidence asks the user to verify in Codex App and resend manually. It never retries automatically. Durable metadata contains only the attempt ID, platform message key, thread/turn baseline, message digest, and status—not the full input needed for later execution.
...
```bash
weclaw codex app          # Initial macOS App sharing setup
weclaw codex cli
weclaw codex cli resume <thread-id>
```

This command pins `--remote` to the verified shared socket and preserves the official interactive TUI, including `resume`, `fork`, and `archive`. `shared` uses standalone Codex and can prepare the same Host through App when WeClaw is stopped. `daemon` uses the official daemon; `managed` uses the configured native command and requires the WeClaw service to prepare its Host first. While the service runs, CLI prepares the Host through a local control endpoint and verifies the socket against local configuration. Custom remote addresses, non-interactive or management commands, unreachable control endpoints, and ambiguous Host identity are rejected. Legacy Codex `type: cli` configuration still migrates to shared app-server.

On a Linux server that cannot reach the standalone installer but can reach an npm registry, use the user-scoped npm channel and explicitly select the managed Host. First run `npm config set prefix "$HOME/.local" --location=user` to pin that user's global package directory, then run `npm install -g @openai/codex@latest`. Set the Codex Agent `command` to the expanded absolute path (for example, `/home/user/.local/bin/codex`), `codex_host_mode` to `managed`, and `codex_app_reuse_daemon` to `false`. Use the same `npm install -g @openai/codex@latest` command for upgrades. Passing `--prefix` only to an individual install does not change npm's default update target and can make `codex update` point at a different global installation. Stop or coordinate a WeClaw restart before changing the installed version; never replace Host lifecycle files while the old Host is running. If coordinated shutdown leaves a transaction that proves the daemon stopped, the first managed startup migrates that transaction only after Desktop absence, current Host identity, and multi-Host preflight all pass; every other topology drift remains fail-closed. `weclaw doctor --fix --components codex` still uses the official standalone installer and never changes an existing installation channel implicitly.
...
- `/cx release`: persist this route's release tombstone and stop its Codex synchronization; the local task, Host, and other routes keep running.
...
With native Feishu progress cards enabled, every supplemental input accepted by an active Codex turn creates a separate relay card beneath that message. The previous card is marked as moved and retains its complete progress, while all later progress and the terminal state go only to the newest card. A successful relay uses the new card itself as confirmation instead of sending another success message; platforms without native progress cards retain the existing text acknowledgement. If card migration fails after Codex accepted the input, WeClaw reports that distinction and never sends the same guide twice.
...
Enable the bot capability in the Feishu developer console, select persistent connection for events and callbacks, and subscribe to `im.message.receive_v1` and `card.action.trigger`. Configure the permissions below and the bot's `allowed_users`, then publish an application version and complete any required approval. Valid credentials alone do not verify event or card callback readiness.

New bots default to `stream` when no progress mode is specified; leaving the mode blank when updating an existing bot preserves its configuration. `feishu login` and `feishu bootstrap` remain compatibility commands; use `feishu add` for initial setup.
...
Before the Agent produces its first effective non-command progress event, a Feishu task card body shows only `思考中.....`; synthetic timer copy such as “waiting for Agent” or “connection healthy” does not replace it. After a Codex commentary, Claude message, plan, file change, or tool summary arrives, the same card shows all progress in the current segment from its first entry onward, without a latest-five preview or expand/collapse controls. User-visible Codex commentary keeps its original text and order; later messages append to the body, while structured events update in place by stable ID. `思考中.....` appears once at the bottom. Capacity checks use the complete card, and oversized progress continues on another card while earlier cards retain their full history. Completion, failure, stop, or relay removes the active indicator and preserves the entire current segment and approval records. Legacy preview cards switch to complete progress when their next progress update arrives. Uninformative Codex command-execution lifecycle entries stay hidden; internal reasoning and status heartbeats do not unlock the waiting card. Progress synchronization failures show an explicit error, and real approvals remain visible.
...
`weclaw doctor` is read-only by default. In addition to the existing configuration checks, it inspects `sqlite3`, Linux `bubblewrap`, Node.js/npm, Codex CLI `app-server` support, official standalone, Claude Code CLI, and the Claude ACP adapter. When `codex_multi_frontend` is enabled, missing standalone is a blocking failure; without strict sharing it remains a managed-compatibility warning. Other missing runtime dependencies for a configured Agent are blocking failures; missing optional Agents or dependencies that affect only the `/cx` catalog or Codex Linux sandbox are warnings.
...
- `codex_multi_frontend` controls the complete-sharing gate before platforms start. For standard native Codex ACP `app-server` configurations in `auto`/`daemon` mode without a custom socket, `run_as_user`, or an explicit App-reuse opt-out, a missing value is normalized to `true`. This requires standalone and forbids compatibility fallback, but does not force Host mode to `daemon`. An explicit `false` retains compatibility without mandatory sharing.
- `codex_host_mode` accepts `auto`, `daemon`, `managed`, and `shared`. On macOS, `auto` selects `shared` when App reuse is enabled and App includes the official Node runtime; other environments retain official-daemon/managed detection, and a custom socket or user selects managed compatibility. Explicit `daemon` never silently switches to shared and retains thread-level Desktop follower coordination on macOS: an active-writer error plus matching thread/turn IPC evidence is required for that binding, while other threads stay on the daemon. Explicit `daemon` and `shared` cannot be combined with `app_server_socket` or `run_as_user`. Unprovable Host authority fails closed instead of starting a second Host.
- Shared managed Hosts, the official daemon, controlled `weclaw codex cli`, and coordinated shutdown run a multi-Host preflight across the inspected UID set before changing Host state. Normal mode is read-only and fails closed on extra or unverifiable Hosts. Explicit `--force` may quit App and stop current-user Codex `app-server` groups only after fresh UID, PGID, start-time, raw-argv, and command-fingerprint checks. No mode kills by name or stale PID alone. This is a point-in-time gate, not a persistent global lock.
- Native Codex `auto`/`daemon` configurations default to `codex_app_reuse_daemon: true` unless sharing or App reuse is explicitly disabled. It is effective only on macOS and manages the launchd environment of future App launches: WeClaw aligns effective `CODEX_HOME`, `CODEX_SQLITE_HOME`, and the control socket, fills only missing path values, and fails closed on explicit conflicts. An unverified official daemon, a forced CLI override, or a running App that still owns an unmarked private `app-server` also fails closed. WeClaw never quits the App for this purpose; fully restart the App after first enablement or a path change. The process never migrates, deletes, or cleans up old database, WAL/SHM, or writer-lock files.
...
| `auto_review` | `workspace-write` + on-request approval, with escalation requests reviewed by the Codex reviewer |
| `full_access` | `danger-full-access` + no approval; trusted, well-isolated environments only |

For scripted configuration, pass the level explicitly—for example, `weclaw config permission --agent codex --level full_access`—and then run `weclaw restart`. This command writes `permission_level` and clears `approval_policy`, `approval_reviewer`, and `sandbox_mode`, because those advanced fields otherwise take precedence over the level mapping. A running Codex Agent does not hot-switch permissions; the restarted Host and subsequent sessions use the new values.

`auto_review` does not expand the `workspace-write` sandbox. WeClaw passes `approvalsReviewer=auto_review` to Codex and never converts a reviewer error or rejection into permission. Chat `/mode yolo` is a separate temporary layer: it is scoped by the real operator and chat route in the current WeClaw process, is cleared on restart, and does not change the global sandbox or `permission_level`. `full_access` does not make WeClaw root or bypass operating-system permissions. Codex can access only what the operating-system user running the systemd service can access, so paths such as `.git` must still be writable by that service user.
...
weclaw status
weclaw restart
weclaw restart --force       # Interrupt tasks, quit App, and force-stop current-user Codex Hosts
weclaw stop
weclaw stop --force          # Interrupt tasks, quit App, and force-stop current-user Codex Hosts
weclaw update
weclaw update --source gitee  # Explicit Gitee mirror for Apple Silicon macOS or Debian amd64
weclaw update --restart
weclaw version
```

Update sources are `auto` (default), `github`, and `gitee`. Gitee binary mirrors are available for `darwin/arm64` and `linux/amd64`. Use `--source` for one command, set `"update_source": "gitee"` in `~/.weclaw/config.json` for a persistent choice, or override it with `WECLAW_UPDATE_SOURCE`. `auto` falls back from GitHub to Gitee only for DNS, connection, TLS, timeout, or HTTP 5xx failures. HTTP 4xx, invalid versions, and SHA-256 failures stop immediately instead of being hidden by a mirror fallback. The updater also rejects a stale mirror that would downgrade the installed stable version.

`weclaw update` returns immediately when the installed version is already current. Configuration and Agent preflight runs only after installing a new version or when `update --restart` is explicitly requested. A running service lets `stop`, `restart`, and `update --restart` use the same Host transaction through the loopback `/api/runtime/restart/prepare` endpoint. Normal mode requires message tasks, controlled CLIs, writer leases, threads, and Host identity to be provably safe. A disconnected client reconnects only to the existing socket and uses the complete `thread/list` snapshot to prove idleness. Stale `running` metadata is repaired only when the PID is absent, the socket has no listener, and process preflight proves no replacement Host exists.

Explicit `restart --force`, `stop --force`, or `update --restart --force` means the operator accepts interruption of local tasks. WeClaw cancels its tasks, requests Codex App to quit, then force-stops current-user Codex `app-server` process groups discovered in a fresh process snapshot, including Hosts with incomplete management identity. An unknown Host must lead its own app-server process group; if it shares a group with a shell or another program, WeClaw refuses the collateral termination. Before the first signal it rereads raw argv and verifies UID, PGID, process start time, and command fingerprint; before escalating to `SIGKILL` it verifies process identity again. It never signals from a stale PID or process name alone. An unreadable process table or argv, identity drift, or an unknown Host stop outcome still fails the complete stop/restart/update instead of starting a service that could create a second Host. If an earlier unknown outcome already left the service non-writable, a later explicit `--force` performs a new scan and convergence attempt instead of reusing the cached transaction as success. Direct `SIGINT` or `SIGTERM` carries no force authorization and preserves unverifiable Hosts under the normal policy.

The bundled `service/weclaw.service` uses `KillMode=process`, so systemd signals only the WeClaw main process and leaves Host coordination to this transaction. Before platform listeners start, the next service reads the protected transaction state, starts the single Host, and verifies that its generation changed. After a real binary update, ordinary preflight, shutdown, or startup failure restores the previous binary and restarts the previous service when necessary. Once an offline force migration journals stop intent, a failure retains the new binary for a subsequent `--force` recovery, because the old binary could ignore an incomplete recovery journal. Without explicit `--restart`, `weclaw update` replaces only the binary and never restarts the service. Update official installations with `weclaw update`. For pre-release testing, use `package --install` below; it uses the same verified, atomic update transaction rather than copying over the installed binary.

If the WeClaw service is already stopped, `weclaw restart` remains read-only by default: it checks Codex App and controlled CLI presence and starts directly, but fails closed instead of stopping an external Host. Explicit `--force` first quits Codex App, then uses a fresh process snapshot to stop current-user Codex `app-server` process groups, including Hosts with incomplete management identity. Before the first signal it rereads raw argv and verifies UID, PGID, process start time, and command fingerprint; before escalating to `SIGKILL` it verifies process identity again. An unreadable process table or argv, or any identity drift, remains fail-closed; WeClaw never kills from a stale PID or process name alone.
When upgrading a service without the coordinated endpoint, such as v0.1.245, run `weclaw restart --force` directly. Use `weclaw stop --force` to stop only, or `weclaw update --restart --force` to update and restart. The new CLI journals recovery intent, verifies and stops the old WeClaw process, quits Codex App, stops the Hosts, and then starts the new service and waits for Host verification when requested. The manual stop/start/restart migration is no longer required. Controlled `weclaw codex cli` sessions must still be closed first. If interrupted, the journal remains and another `stop --force` or `restart --force` resumes recovery; ordinary startup cannot bypass an incomplete stop. A systemd-managed legacy service uses `weclaw.service` with `KillMode=process`.

Without `--force`, legacy migration still requires App/CLI absence and a drain endpoint that proves no active tasks. Versions without that endpoint require the force migration above. HTTP 404 is treated as capability negotiation, with no JSON parsing or compensation request to the missing transaction endpoint.
...
The repository currently uses Go 1.26.6. No publicly pullable container image is currently published in sync with this maintained distribution.

`scripts/release.sh` is the only authoritative stable-release entrypoint. The manual GitHub Actions Release workflow checks out clean `main` and delegates to that script instead of maintaining a second test, build, or upload pipeline. GitHub Release remains authoritative for versions and builds; CI and the release script build and upload only `weclaw_darwin_arm64`, `weclaw_linux_amd64`, and their original `checksums.txt`. Only after that release is public and verified are reversible `.gz` representations of both binaries, plus the same original checksums, mirrored to [Gitee](https://gitee.com/jimdeng891/weclaw). CI and Linux inject `GITEE_TOKEN`; a local macOS release falls back to the `weclaw-gitee-release` login-keychain item when the environment variable is absent. Before any Git push, the mirror validates the token against the exact target repository through an Authorization header stored only in its protected temporary directory. The mirror checks final attachment names and counts after upload without downloading the same files again; installers and updaters still verify the selected binary against the authoritative checksum. A mirror failure visibly fails the release job without deleting the already verified GitHub Release; the manual `Repair Gitee Mirror` workflow idempotently resumes missing attachments from that GitHub Release and rechecks the attachment inventory.

### Package, Test Locally, Then Publish

```bash
# Commit local changes first; pushing and release credentials are not required yet.
scripts/release.sh package --next-patch --install
# Restart explicitly when safe to perform local testing.
# After testing, push the same commit and publish the exact packaged version.
git push origin main
scripts/release.sh publish vX.Y.Z
```

Omit `--install` to only package. Full validations run before generating both binaries and `checksums.txt` in `dist/vX.Y.Z/`. The adjacent `vX.Y.Z.package.json` records the version, commit and SHA-256 hashes; it is not uploaded as a release asset. Packaging requires a clean committed tree, but no remote synchronization. An unchanged sealed package can be reused. After changing source, commit it and move the old package directory and manifest aside before rebuilding; do not edit the manifest to reuse old validation results.

`--install` invokes the packaged updater against the existing installation in PATH, including installations too old to support the new flags. Newer installations also accept `weclaw update --from-package /path/to/dist/vX.Y.Z`. Local installation verifies hashes, platform and executable version, rejects downgrades, and reuses atomic replacement and rollback on preflight failure. It does not query remote versions, restart services, or automatically roll back subsequent manual testing. `--target` is only valid with a local package and must match an existing installation and any running service path.

`publish` checks that the package matches clean `main` and `origin/main`, then uploads the original artifacts without rebuilding them. `publish vX.Y.Z --dry-run` checks prerequisites without uploading. Draft asset checks and an update smoke test from the previous stable release precede promotion to latest. The manual Release workflow likewise separates `package` and `publish`; publishing requires a successful `package_run_id` and restores its artifacts at the same commit.
