...
On the first upgrade from a version that does not provide this coordinated endpoint, the updater itself is still the old process, so that invocation of `update --restart` cannot prove the new transaction. Perform `weclaw update` without `--restart`, fully quit Codex App and controlled CLIs, then run `weclaw restart` with the newly installed binary.
