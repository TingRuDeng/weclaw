...
On a Linux server that cannot reach the standalone installer but can reach an npm registry, use the user-scoped npm channel and explicitly select the managed Host: run `npm install -g --prefix "$HOME/.local" @openai/codex@latest`, then set the Codex Agent `command` to the expanded absolute path (for example, `/home/user/.local/bin/codex`), `codex_host_mode` to `managed`, and `codex_app_reuse_daemon` to `false`. Use the same npm command for upgrades. Stop or coordinate a WeClaw restart before changing the installed version; never replace Host lifecycle files while the old Host is running. If coordinated shutdown leaves a transaction that proves the daemon stopped, the first managed startup migrates that transaction only after Desktop absence, current Host identity, and multi-Host preflight all pass; every other topology drift remains fail-closed. `weclaw doctor --fix --components codex` still uses the official standalone installer and never changes an existing installation channel implicitly.
