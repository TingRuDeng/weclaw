...
If the WeClaw service is already stopped, `weclaw restart` keeps its compatibility behavior of starting directly after checking Codex App and controlled CLI presence. With no old service available to perform the loopback Host transaction, it does not claim to have rotated an independently running external Host.
