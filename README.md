...
`weclaw update` returns immediately when the installed version is already current. Configuration and agent preflight runs only after installing a new version or when `update --restart` is explicitly requested. `restart` and `update --restart` use the same coordinated restart transaction: they first hold an exclusive Codex frontend lease, close message admission and drain tasks through the loopback `/api/runtime/restart/prepare` endpoint, then require Codex App to be fully quit and prove that no controlled `weclaw codex cli`, writer lease, or active/unknown thread remains before stopping the identity-verified official daemon or WeClaw-managed Host. If Codex App or a controlled CLI is still running, the command refuses before stopping WeClaw. WeClaw uses protected IPC and same-user main-process names only for conservative presence detection; it never terminates Codex App by process name or quits it automatically. `--force` only interrupts WeClaw-owned tasks and cannot bypass Host safety gates. Before platform listeners start, the replacement service reads the protected restart state, starts the one Host, and verifies that its generation changed. Verification failures remain fail-closed, while an outer shutdown failure reconstructs the previous Host before reopening admission. A systemd-managed instance remains owned by systemd instead of spawning a private daemon. If preflight fails after installing a new version, WeClaw restores the previous binary. During `update --restart`, safety-check, shutdown, or startup failures likewise restore the previous binary and restart the previous service if it had already stopped; rollback failures are reported together with the original update error. Without an explicit `--restart`, `weclaw update` replaces only the binary and never restarts the service. Update official installations with `weclaw update`; never overwrite the binary in PATH with a local build.
