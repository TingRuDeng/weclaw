...
Use local Codex and Claude remotely from WeChat or Feishu while keeping real workspace and session context, live progress, approvals, and results. Codex always has one authoritative Host. In the complete multi-frontend mode, Codex App, the controlled CLI, WeChat, and Feishu all connect to one verified official daemon and continue the same thread.
...
- **Use one Codex runtime boundary**: a verified official daemon is the sole Host in complete sharing mode; App is a frontend and history view, active-turn inputs follow app-server acceptance order, and writer leases serialize new turns.
...
The default `codex_host_mode: auto` on macOS freezes the Host topology when the Agent is constructed. An identity-verified daemon on the fixed control socket, or an available official standalone installation under `CODEX_HOME`, selects the official daemon as the sole Host. Codex App history visibility never changes that authority to Desktop. Only when standalone is unavailable does `auto` retain the private App Host or WeClaw-managed Host as a compatibility path; that path does not promise complete App/CLI/Feishu sharing and never starts a second Host to simulate synchronization. Explicit `daemon` mode may use Desktop IPC for frontend probes and coordination, but can never select the App Host. Daemon startup or identity failures remain fail-closed.

Set `"codex_multi_frontend": true` on the native Codex Agent when complete sharing is a required capability. This forces the effective Host mode to `daemon` and enables macOS App daemon reuse. If official standalone is missing, `weclaw doctor` reports a blocking failure and `weclaw start` stops before any platform starts with guidance to run `weclaw doctor --fix --components codex`; it never falls back to managed. Once the file exists, startup also synchronously starts and validates the daemon, App reuse, and Host identity before registering platforms, so a later validation failure cannot leave messaging online first. The setting cannot be combined with `codex_host_mode: managed`, a custom `app_server_socket`, `run_as_user`, or `codex_app_reuse_daemon: false`. Omitting it preserves the legacy `auto` behavior. Explicit `false` prevents configuration normalization from re-enabling App daemon reuse.

Except for explicit `--force`, shared managed Hosts, the official daemon, and controlled `weclaw codex cli` sessions run a read-only multi-Host preflight before startup, attachment, or coordinated shutdown. On macOS, candidate argv is read with `kern.procargs2`; on Linux, it comes from `/proc/<pid>/cmdline`. The scan groups the Node wrapper and native child by PGID. Extra Hosts, unreadable process data, or unverifiable authority fail closed. `codex --remote`, help, daemon, proxy, and schema-generation tooling are not Hosts. Read-only preflight stops no process; the force path's extra termination authority and live identity checks are described under Run and Update. No path kills an existing App or unknown process by name. This is a point-in-time gate, not a persistent global lock across sockets or `CODEX_HOME` values.

“Codex App and daemon are both present” assumes that the App is connected to that same official daemon. Native Codex shared app-server configurations now default to `codex_app_reuse_daemon: true`. WeClaw first verifies the official daemon and proves that its fixed control socket exactly matches the `CODEX_HOME` used by the App. It then enables the official local-daemon entrypoint in the current macOS user's launchd environment for future App launches; the App bundle and signature are never modified. An already-running App that still owns a private `app-server` child is never force-quit: the Codex Agent fails closed and asks for a full App quit and reopen. The first upgrade to this behavior therefore requires one App restart. If the App still falls back to a private Host afterward, update both Codex App and the standalone CLI and remove conflicting `CODEX_CLI_PATH` or `CODEX_APP_SERVER_FORCE_CLI=1` launch overrides. Once attached, the App shows the daemon's session catalog. A separate WeClaw `CODEX_SQLITE_HOME` can make that catalog look different from the previous private App Host, but the old catalog is not deleted. Explicit `false` removes the launchd flag managed by WeClaw and likewise takes effect only after the next App launch.

The common, primary collaboration shapes are “Feishu + Codex App” and “Feishu + controlled Codex CLI,” provided every frontend connects to the same verified official daemon. After Feishu binds an idle thread, synchronization starts automatically when App or CLI later starts a task. When Feishu selects an active thread, WeClaw first restores existing user-visible natural-language progress and then follows later progress. A regular message carries the current `turnId` into the active turn; a new turn starts only when the thread is idle. Complete sharing also allows App, CLI, and Feishu to open the same thread at once. Upstream request acceptance still determines order; WeClaw does not claim client-level exclusivity or precise ownership.

A session binding represents only the thread selected by the current frontend window; it is not writer ownership. `/cx switch` and `/cx new` first prove that the single Host is reachable and validate the target with a lightweight `thread/read`. An unreadable Host or target preserves the previous binding; a readable target is committed immediately. The v14 `preparing/ready` fields remain as follower-observer synchronization state and for rollback compatibility, but no longer authorize regular messages. History replay, progress-card, or exact-turn observer failures mark progress synchronization as “syncing” or “degraded” without rolling back a valid binding.

Multiple bound Feishu routes may observe and write the same thread. Each route independently receives progress, approval or question displays, and one terminal result. A single-decision broker keyed by `(thread, turn, request)` accepts only one approval or structured-answer submission; every other display converges to externally resolved. On restart, WeClaw restores Host authority, history and pending interactions, observer readiness, and finally terminal-outbox delivery. Observer recovery affects synchronization and terminal catch-up, not whether a new input may use an available runtime. A durable follower also records the exact `allowed_users` identity authorized for that bot, so revocation clears only that route and blocks its guarded pending results. An idle binding performs only lightweight reads and does not use `thread/resume` to retain a long-lived subscription. Once an active turn is confirmed terminal, WeClaw sends `thread/unsubscribe` once while preserving the durable binding, so an idle Feishu selection does not keep Codex App showing that the session is open elsewhere. `/cx release` stops observation and interaction delivery only after the durable release tombstone is saved; when no other frontend or active task needs observation, it sends `thread/unsubscribe` only on the current app-server connection. Release never interrupts the active turn, restarts the Host, or affects another route.
...
When Codex App is a frontend on the same official daemon, Feishu can select existing sessions, continue active turns, receive progress and interactions, use `/stop`, and change the current thread's model or reasoning level. A regular Feishu message does not wait for observer `ready`: when the runtime is readable it enters an App-started active turn, and App may likewise continue a Feishu-started turn. An App that still owns a private app-server is a compatibility path rather than complete sharing; fully quit and reopen it so it reconnects to the verified daemon instead of attempting to hot-migrate an active turn.

Approvals and structured questions on the same official daemon are displayed to every synchronized observer, but the broker accepts only one decision. When Codex App or another route resolves the request first, the daemon's `serverRequest/resolved` notification idempotently closes the other displays without a default denial or duplicate response. Every regular input re-reads the authoritative thread state: active uses `turn/steer` with `expectedTurnId`, idle uses `turn/start`, and a not-loaded thread is resumed only immediately before writing. Deterministic start/steer races re-read state and submit that input once. A genuinely unavailable Host rejects immediately; WeClaw neither queues nor stores the input for delayed execution.

An App process, secure IPC, or visible history is only frontend evidence and cannot change official-daemon authority. The exact error `no-client-found: thread stream owner became unavailable` belongs to the private-App compatibility path; WeClaw never starts a second Host in response. For complete sharing, first fully reopen App on the same official daemon, then use `/cx status` to inspect binding, runtime channel, and progress synchronization separately. “Progress synchronization: degraded” does not make the session unwritable, and the session never needs to be deleted or recreated.
...
This command pins `--remote` to WeClaw's single verified shared Host socket. It allows only the interactive TUI and its `resume`, `fork`, and `archive` operations; custom `--remote`, non-interactive, and management subcommands are rejected. `daemon` mode continues to use the official standalone Codex binary and may start that daemon directly when WeClaw is stopped and Codex App is absent. `managed` mode uses the native Codex command in the Agent configuration, so an npm-installed `@openai/codex` is supported, but WeClaw must already be running to create and verify the Host; the CLI cannot start or orphan a second Host. While the service is running, the CLI first asks it through a loopback-only control endpoint to prepare the Host, then proves that the returned socket exactly matches the local configuration. App-as-Host authority, an unreachable service control endpoint, ambiguous Host identity, or a stopped managed service all fail closed. App visibility alone is no longer a rejection when the service has already proved that the shared Host is authoritative. Legacy Codex `type: cli` configuration migrates to the shared app-server, and the old independent `codex exec` session mode is removed.
...
- Native Codex `auto`/`daemon` configurations default to `codex_app_reuse_daemon: true`. It is effective only on macOS and manages only the launchd environment of future App launches. An unverified official daemon, a `CODEX_HOME`/control-socket mismatch, a forced CLI override, or a running App that still owns a private `app-server` all fail closed. WeClaw never quits the App for this purpose; fully restart the App once after first enablement.
