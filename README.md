...
- **Use a controlled Codex runtime boundary**: a verified official daemon is the default shared Host; if App Code Mode owns one active thread writer, WeClaw collaborates only through verified follower IPC and never starts another Host itself.
...
The default `codex_host_mode: auto` on macOS freezes the shared-Host topology when the Agent is constructed. An identity-verified daemon on the fixed control socket, or an available official standalone installation under `CODEX_HOME`, selects the official daemon as the default shared authority. Codex App history visibility alone never changes that authority. Newer Codex App Code Mode sessions may run in an App-owned, writer-locked Host; when the daemon explicitly reports that a thread already has an active writer, WeClaw uses verified Desktop follower IPC only for that active thread while other threads remain on the daemon. Only when standalone is unavailable does `auto` retain the private App Host or WeClaw-managed Host compatibility path. WeClaw never starts a second Host merely to simulate synchronization. Explicit `daemon` mode keeps the same targeted Desktop coordination, and daemon startup or identity failures remain fail-closed.
...
“Codex App and daemon are both present” means the App inherited the official local-daemon configuration; it no longer assumes every Code Mode thread is held by the daemon process. WeClaw verifies the daemon and matching `CODEX_HOME` control socket before enabling `CODEX_APP_SERVER_USE_LOCAL_DAEMON=1` for future App launches. An App-bundled `features.code_mode_host=true` helper may still hold a Code Mode thread's writer lock without invalidating daemon reuse. WeClaw uses its follower only after `thread/resume` reports an active writer and Desktop IPC confirms the same thread and an exact active turn id. An unmarked private `app-server` still fails closed and requires a full App restart. Conflicting `CODEX_CLI_PATH` or `CODEX_APP_SERVER_FORCE_CLI=1` launch overrides remain unsupported.

The common collaboration shapes are “Feishu + Codex App” and “Feishu + controlled Codex CLI.” Idle threads continue on the verified official daemon. If an active App Code Mode thread is held by the App helper, Feishu joins that turn through the Desktop follower. A regular message carries the current `turnId` into the active turn; a new turn starts only when the thread is idle. App, CLI, and Feishu may open the same thread at once, and upstream request acceptance determines ordering.
