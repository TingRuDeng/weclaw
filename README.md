...
- `codex_host_mode` accepts `auto`, `daemon`, and `managed`. On macOS, the default `auto` keeps a verified running official daemon first; with no running daemon it reuses a running Codex App, then selects an available standalone daemon only when the App is absent, and otherwise uses the compatible `managed` backend. Platforms without the Desktop bridge select an available official daemon or fall back to `managed`. An unidentified official socket or a running App whose secure IPC cannot be reached fails closed instead of starting a second Host. Explicit `daemon` never falls back and cannot be combined with `app_server_socket` or `run_as_user`.
