...
Multiple bound Feishu routes may observe and write the same thread. Each route independently receives progress, approval or question displays, and one terminal result. A single-decision broker keyed by `(thread, turn, request)` accepts only one approval or structured-answer submission; every other display converges to externally resolved. On restart, WeClaw restores Host authority, history and pending interactions, observer readiness, and finally terminal-outbox delivery. Observer recovery affects synchronization and terminal catch-up, not whether a new input may use an available runtime. A durable follower also records the exact `allowed_users` identity authorized for that bot, so revocation clears only that route and blocks its guarded pending results. `/cx release` stops observation and interaction delivery only after the durable release tombstone is saved; when no other frontend or active task needs observation, it sends `thread/unsubscribe` only on the current app-server connection. Release never interrupts the active turn, restarts the Host, or affects another route.
