...
On the first upgrade from a version that does not provide this coordinated endpoint, the new binary in PATH and the old service still running in memory have different capabilities. If the new CLI receives HTTP 404 for the coordinated endpoint, it fails closed before touching any process, reports the running service version and the one-time migration sequence, does not parse the plain-text `404 page not found` body as JSON, and does not send compensation for a transaction that never started. Wait for every task to finish and fully quit Codex App and controlled CLIs, then run `weclaw stop`, `weclaw start`, and `weclaw restart` in that order. The first start moves the service process to the new version; only the final restart performs the complete Host stop and generation verification. Do not describe the intermediate offline start as a completed coordinated transaction.
