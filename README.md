...
Shared managed Hosts, the official daemon, and controlled `weclaw codex cli` sessions run a read-only multi-Host preflight before startup, attachment, or coordinated shutdown. Normal mode scans the effective UID; `run_as_user` mode also scans the target UID and root only to recognize the waiting sudo wrapper. On macOS, candidate argv is read with `kern.procargs2`; on Linux, it comes from `/proc/<pid>/cmdline`, preserving spaces and argument boundaries. The scan groups the Node wrapper and native child by PGID and permits only the authoritative process group proven by protected metadata or an official lifecycle PID. An extra `codex app-server`, an unreadable process table or candidate argv, or an unverifiable authority fails the operation closed; errors contain only a redacted category, PGID, and bounded PID list, never the full command. `codex --remote`, help, daemon, proxy, schema-generation tooling, and near matches are not Hosts. The preflight itself stops no process. If a post-start check detects a race, the compatibility managed path reclaims only the new Host that WeClaw can prove it just created; it never kills an existing Codex App or unknown process by name. This is a point-in-time gate, not a persistent global lock across sockets or `CODEX_HOME` values; a Host started externally afterward is detected at the next gate.
...
Session selection is a durable two-phase attach. `preparing` means only that this route's workspace/thread intent has been saved; it does not mean the route is writable or synchronized, and regular messages fail closed. WeClaw must prove the exact Host generation, read authoritative history, and, for an active target turn, create its native progress card and exact-turn observer. Only then does a revision/turn/generation CAS commit `ready` and report “switched and bound.” Any failed step preserves the `preparing` intent for idempotent background retry. The original Feishu result card converges only after `ready`; card-update retries remain independent from runtime recovery.

Multiple bound Feishu routes may observe the same thread. Each route independently receives progress, approval or question displays, and one terminal result. A single-decision broker keyed by `(thread, turn, request)` accepts only one approval or structured-answer submission; every other display converges to externally resolved. On restart, WeClaw restores Host authority, history and pending interactions, observer readiness, and finally terminal-outbox delivery. Guarded results cannot leave before that route is `ready`. A durable follower also records the exact `allowed_users` identity authorized for that bot, so revocation clears only that route and blocks its guarded pending results. `/cx release` stops observation and interaction delivery only after the durable release tombstone is saved; a persistence failure keeps the binding. Release never interrupts the active turn, restarts the Host, affects another route, or leaves a read-only observer.
...
When Codex App is a frontend on the same official daemon, Feishu can select existing sessions, continue active turns, receive progress and interactions, use `/stop`, and change the current thread's model or reasoning level. After the route reaches `ready`, a regular Feishu message enters the App's active turn instead of waiting or starting a second turn. An App that still owns a private app-server is a compatibility path rather than complete sharing; fully quit and reopen it so it reconnects to the verified daemon instead of attempting to hot-migrate an active turn.

Approvals and structured questions on the same official daemon are displayed to every ready observer, but the broker accepts only one decision. When Codex App or another route resolves the request first, the daemon's `serverRequest/resolved` notification idempotently closes the other displays without a default denial or duplicate response. Temporarily unreadable state remains fail-closed and keeps both the binding and pending request. Only a disconnect after writing, or a response timeout, is classified as unknown delivery. The five-minute wait on the compatibility Desktop IPC path likewise triggers revision-guarded state review and never an automatic denial.

An App process, secure IPC, or visible history is only frontend evidence. It does not prove that a WeClaw route is `ready` and cannot change official-daemon authority. The exact error `no-client-found: thread stream owner became unavailable` belongs to the private-App compatibility path. WeClaw keeps that binding in `preparing` and retries idempotently without starting a second Host or reporting a false attach. For complete sharing, first fully reopen App on the same official daemon, then use `/cx status` to confirm that the route is ready. The session never needs to be deleted or recreated.
...
- Shared managed Hosts, the official daemon, controlled `weclaw codex cli`, and coordinated shutdown run a read-only multi-Host preflight across the inspected UID set before changing Host state; `run_as_user` additionally covers the target UID and root sudo wrapper. Extra `app-server` groups or an unreadable process table or candidate argv fail closed with redacted PGID/PID details, and WeClaw never kills existing or unknown processes by name. This is a point-in-time gate, not a persistent global lock.
...
weclaw status
weclaw restart
weclaw restart --force       # Interrupt only WeClaw-owned active tasks
...
`weclaw update` returns immediately when the installed version is already current. Configuration and agent preflight runs only after installing a new version or when `update --restart` is explicitly requested. A running service that supports the coordinated endpoint lets `stop`, `restart`, and `update --restart` use the same Host safety transaction through the loopback `/api/runtime/restart/prepare` endpoint: they first hold an exclusive Codex frontend lease, close message admission and drain tasks, then require Codex App to be fully quit and prove that no controlled `weclaw codex cli`, writer lease, or active/unknown thread remains before stopping the identity-verified official daemon or WeClaw-managed Host. If an ordinary restart finds that its client connection to the shared Host was lost, it reconnects only to the existing socket and reads the complete authoritative `thread/list` state before deciding. Only stale active fields on `CodexRuntimeWeClaw` bindings are cleared when the Host proves every thread idle; Desktop, unknown, or conflict bindings, reconnect failures, and real active threads still block the restart. `stop` sends `SIGTERM` to WeClaw only after preparation succeeds; preparation failure leaves the service running, and an outer stop failure reconstructs the previous Host before reopening admission. A direct `SIGINT` or `SIGTERM`, including systemd stop or an unexpected message-bridge exit, also attempts the same transaction inside the service. If that external stop can no longer be refused but Host state is unsafe or unknown, WeClaw exits, records why the Host was preserved, and never kills it blindly. The bundled `service/weclaw.service` uses `KillMode=process` so systemd signals only the WeClaw main process and lets this transaction manage the Host; custom units must preserve that setting instead of the default `control-group`. If Codex App or a controlled CLI is still running, CLI management commands refuse before stopping WeClaw. WeClaw uses protected IPC and same-user main-process names only for conservative presence detection; it never terminates Codex App by process name or quits it automatically. Forced task draining only interrupts WeClaw-owned tasks and cannot bypass Host safety gates. Before platform listeners start, the next service reads the protected transaction state, starts the one Host, and verifies that its generation changed. Verification failures remain fail-closed. A systemd-managed instance remains owned by systemd for stop and restart instead of spawning a private daemon. If preflight fails after installing a new version, WeClaw restores the previous binary. During `update --restart`, safety-check, shutdown, or startup failures likewise restore the previous binary and restart the previous service if it had already stopped; rollback failures are reported together with the original update error. Without an explicit `--restart`, `weclaw update` replaces only the binary and never restarts the service. Update official installations with `weclaw update`; never overwrite the binary in PATH with a local build.

If the WeClaw service is already stopped, `weclaw restart` remains read-only by default: it checks Codex App and controlled CLI presence and starts directly, but fails closed instead of stopping an external Host. Only the explicit `--stop-conflicting-codex-hosts` flag may stop an identity-verified official daemon, private Codex App Host, or WeClaw-managed Host before startup. PID, PGID, UID, process start time, command fingerprints, and protected metadata must all match; otherwise the command refuses to stop the process. `--force` only interrupts WeClaw-owned tasks and never authorizes stopping Codex Hosts or Codex App.
...
`scripts/release.sh` is the only authoritative stable-release entrypoint. The manual GitHub Actions Release workflow checks out clean `main` and delegates to that script instead of maintaining a second test, build, or upload pipeline. GitHub Release remains authoritative for versions and builds; CI and the release script build and upload only `weclaw_darwin_arm64`, `weclaw_linux_amd64`, and their original `checksums.txt`. Only after that release is public and verified are reversible `.gz` representations of both binaries, plus the same original checksums, mirrored to [Gitee](https://gitee.com/jimdeng891/weclaw). The mirror checks final attachment names and counts after upload without downloading the same files again; installers and updaters still verify the selected binary against the authoritative checksum. A mirror failure visibly fails the release job without deleting the already verified GitHub Release; the manual `Repair Gitee Mirror` workflow idempotently resumes missing attachments from that GitHub Release and rechecks the attachment inventory.
