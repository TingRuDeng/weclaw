...
Set `"codex_multi_frontend": true` to prepare and verify the shared service before messaging platforms start. macOS `auto` selects the App adapter; other platforms use the official daemon. Missing standalone binaries, conflicting Hosts, or unverifiable identity block startup. This setting cannot be combined with `managed`, a custom `app_server_socket`, `run_as_user`, or `codex_app_reuse_daemon: false`.
...
- `codex_multi_frontend: true` is the user-intent switch for complete sharing: it forces the official daemon, requires standalone to be installed, and forbids compatibility fallback. Omitting it preserves the legacy `auto` compatibility policy.
- `codex_host_mode` accepts `auto`, `daemon`, and `managed`. On macOS, the default `auto` freezes to `daemon` whenever an official daemon is already running or standalone is available; a running App never changes that choice to Desktop. Only when standalone is unavailable does `auto` enter the private-App or `managed` compatibility path. Explicit `daemon` retains Desktop IPC coordination but cannot switch to the App Host; it never falls back and cannot be combined with `app_server_socket` or `run_as_user`. Platforms without Desktop coordination likewise select an available official daemon or fall back to `managed`. An unidentified official socket, unreachable private-App IPC, or unprovable Host authority fails closed instead of starting a second Host.
...
- Native Codex `auto`/`daemon` configurations default to `codex_app_reuse_daemon: true`. It is effective only on macOS and manages the launchd environment of future App launches: WeClaw aligns effective `CODEX_HOME`, `CODEX_SQLITE_HOME`, and the control socket, fills only missing path values, and fails closed on explicit conflicts. An unverified official daemon, a forced CLI override, or a running App that still owns an unmarked private `app-server` also fails closed. WeClaw never quits the App for this purpose; fully restart the App after first enablement or a path change. The process never migrates, deletes, or cleans up old database, WAL/SHM, or writer-lock files.
