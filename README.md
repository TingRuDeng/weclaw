...
With the default `codex_host_mode: auto` on macOS, an official standalone daemon already running on its fixed control socket remains the single Host after its identity is verified, even when Codex App is also running. Codex App, the controlled CLI, Feishu, and WeChat can therefore reuse the same threads. Without a running daemon, WeClaw uses protected Desktop IPC when the App is running; when the App is absent, it connects to or starts the official daemon, with the WeClaw-managed Host as the compatibility backend. When a managed Host is already running, a durable Feishu follower detects an App launched later and waits until every source-Host thread is idle with no writer lease. It then stops the source Host, commits Desktop authority, registers the follower, and loads complete history; the target App thread may already have an active turn. A busy source Host, ambiguous daemon identity, an ambiguous App endpoint, or an uncertain stop preserves the binding and retries without parallel writes or another session selection. Explicit `daemon` mode on macOS also installs Desktop IPC for frontend probes and required Host coordination; it cannot select the App as Host, and daemon startup or identity failures remain fail-closed.
...
The supported collaboration shapes are “Feishu + Codex App” and “Feishu + controlled Codex CLI.” Both frontends bind the same thread, observe task state from the same Host, and can continue the task. After Feishu binds an idle thread, synchronization starts automatically when App or CLI later starts a task. When Feishu selects an active thread, WeClaw first restores its existing user-visible natural-language progress and then follows later progress. A regular message uses the current `turnId` to join the active turn; a new turn is started only when the thread is idle. If App, CLI, and Feishu all happen to open the same thread, upstream request acceptance still determines order, but WeClaw does not claim client-level exclusivity or precise ownership.

Session binding and runtime availability are separate outcomes. WeClaw first persists the workspace/thread and synchronization endpoint selected by the current message route, then asks the current Host to load its state and establish synchronization. “Selected; waiting for runtime” means that the target was remembered but is not writable yet. Regular messages are blocked in that state; use `/cx status` to inspect it. Background reconciliation keeps retrying, so opening the exact thread in App restores synchronization without another `/cx switch`. For a Feishu session-card selection, successful recovery updates the original switch-result card to “switched and bound” instead of sending a duplicate notification. A card-update failure is retried independently and does not block the recovered session synchronization.

The Feishu binding is durable. After a WeClaw restart or short disconnect, observation is attached again when platform delivery returns; a failed recovery is observable and never stops the local task or pretends synchronization succeeded. Multiple Feishu routes may bind the same thread and independently receive progress and one final result; an approval or question is assigned to only one execution frontend to prevent duplicate answers. A durable follower records the exact `allowed_users` identity authorized for that bot; revoking it clears that route's follower and blocks its guarded pending results, and reauthorization requires selecting the session again. `/cx release` unbinds only the current message route, stops progress, approvals, questions, and final results from returning to that route, and freezes its current card without a terminal state. It does not affect other routes, interrupt the active turn, restart the Host, or retain a read-only observer. Local Codex keeps running; selecting the thread again resumes from the latest authoritative snapshot.
...
An App process and a secure IPC endpoint do not prove that WeClaw is registered as a synchronization follower for the target thread. The exact error `no-client-found: thread stream owner became unavailable` means that Router found the owner handler but the owner could not yet deliver a complete snapshot to this follower. A common cause is WeClaw joining after App opened the session and missing the one-shot following-status request. WeClaw keeps the committed Feishu binding, proactively and idempotently registers the follower, and retries without starting a second Host. Normally no second selection or App restart is needed. If recovery still fails, confirm that App has the exact session open under the target workspace, then quit and reopen App. The session does not need to be recreated or deleted.
...
- `codex_host_mode` accepts `auto`, `daemon`, and `managed`. On macOS, the default `auto` keeps a verified running official daemon first; with no running daemon it reuses a running Codex App, then selects an available standalone daemon only when the App is absent, and otherwise uses the compatible `managed` backend. Explicit `daemon` retains Desktop IPC coordination and thread return on macOS but cannot switch to the App Host; it never falls back and cannot be combined with `app_server_socket` or `run_as_user`. Platforms without Desktop coordination select an available official daemon or fall back to `managed`. An unidentified official socket, or an App whose secure IPC is unreachable while `auto` is selecting the App Host, fails closed instead of starting a second Host.
