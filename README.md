...
weclaw restart --stop-conflicting-codex-hosts  # Explicitly stop verified conflicting Codex Hosts
weclaw stop
...
The bundled `service/weclaw.service` uses `KillMode=process`, so systemd signals only the WeClaw main process and leaves Host coordination to this transaction. Before platform listeners start, the next service reads the protected transaction state, starts the single Host, and verifies that its generation changed. After a real binary update, preflight, shutdown, or startup failure restores the previous binary and restarts the previous service when necessary. Without explicit `--restart`, `weclaw update` replaces only the binary and never restarts the service. Update official installations with `weclaw update`. For pre-release testing, use `package --install` below; it uses the same verified, atomic update transaction rather than copying over the installed binary.

If the WeClaw service is already stopped, `weclaw restart` remains read-only by default: it checks Codex App and controlled CLI presence and starts directly, but fails closed instead of stopping an external Host. Explicit `--stop-conflicting-codex-hosts` stops only Hosts whose protected metadata or lifecycle identity is verified. Explicit `--force` first quits Codex App, then uses a fresh process snapshot to stop current-user Codex `app-server` process groups, including Hosts with incomplete management identity. Before the first signal it rereads raw argv and verifies UID, PGID, process start time, and command fingerprint; before escalating to `SIGKILL` it verifies process identity again. An unreadable process table or argv, or any identity drift, remains fail-closed; WeClaw never kills from a stale PID or process name alone.
On the first upgrade from a version that does not provide this coordinated endpoint, the new binary in PATH and the old service still running in memory have different capabilities. If the new CLI receives HTTP 404 for the coordinated endpoint, it recognizes a capability negotiation result before parsing the body, does not treat plain-text `404 page not found` as JSON, and does not send compensation for a transaction that never started. It then performs a guarded legacy migration: only after Codex App and controlled CLIs are fully quit, an exclusive frontend lease is held, and the old `/api/runtime/drain` endpoint confirms `draining=true`, `active_tasks=0`, and `remaining_tasks=0` does it close admission and stop WeClaw itself. Any migration gate failure remains fail-closed and leaves every process running. This migration stop does not rotate the Codex Host; run `weclaw start` to move the service process to the new binary, then run `weclaw restart` for the complete Host stop and generation verification.
