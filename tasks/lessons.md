...
- 正确做法：最终结果独立发送时，进度流只更新为状态完成并保留已有结构化过程；没有结构化进度时显示明确说明，不重复写入最终回答或“正在发送结果”类正文。
...
- 当前规则：原生 stream 任务立即创建进度卡，第一条有效非命令进度前只显示 `思考中.....`；没有结构化进度的短任务终态保留最后有效状态或明确的“未产生结构化进度记录”，并独立发送最终结果。排队与接管仍必须继承当前账号已解析的进度配置。
