...
- 远程边界：一个主机只有一个全局活动账号。群聊只能看到当前脱敏标签；当前平台或机器人 `allowed_users` 身份可在私聊中列出或切换，飞书确认能力必须绑定机器人、操作者、route、目标 profile 和 revision，并具备 TTL 与重复点击幂等。
...
- 交互边界：立即收纳卡片只能表述“已受理”，不能用“已选择成功”暗示后端已经完成；工作空间加载、会话或账号切换、其他耗时操作分别只提示“正在加载中”“正在切换中”“正在处理中”，不得向用户暴露 owner、runtime、共享 Host 或写入门禁等内部实现术语。最终成功或失败必须由业务结果明确反馈。
...
## 2026-07-12 飞书即时任务卡片终态（历史，已由独立结果卡与 outbox 取代）

- 触发条件：飞书需要在 Agent 首个进度事件前给出可见反馈，并把最终回答收敛到同一张卡片。
- 当前规则：原生 stream 任务登记后立即创建只显示 `思考中.....` 的进度卡；终态只改变卡片状态并保留过程，完整成功、失败或停止结果由独立静态 Markdown 结果卡交付。checkpoint 与结果两路分别持久化、独立尝试和重试，任一路失败不得阻塞另一路。
- 当时规则：即时反馈与终态通知必须分离；任务登记后立即建卡，成功写入完成终态后不再补发“请查看上方卡片”，失败或停止只有在卡片成功写入对应终态后才补发简短通知。
...
## 2026-07-11 Codex App 跨进程控制提示（历史，已由 Desktop follower 取代）

- 触发条件：用户从飞书或微信切换到 Codex App 独立进程正在执行的会话。
- 当前规则：App Host 的 Desktop follower 已支持目标 active turn 的 `turn/interrupt`；只有 follower 未暴露的新建、归档、重命名、完整模型目录、账号和额度能力才提示用户回到 Codex App。
- 当时规则：切换反馈和 `/ps` 只展示任务、进度和结果回传，不主动提示“需在 Codex App 中操作”；只有用户实际发送 `/stop` 时才说明跨进程停止限制。
...
## 2026-07-11 Codex 暂存消息自动续跑（历史，Codex 当前直接 steer）

- 触发条件：Codex 正在执行任务，用户发送第二条普通消息且未选择 `/guide`、`/cancel` 或 `/stop`。
- 当前规则：Codex 活动 turn 的普通补充输入直接 `turn/steer`，不进入 WeClaw 私有暂存队列；Claude 仍允许同 route 最多暂存一条并在当前任务终态后自动续跑。
- 当时规则：上一任务进入终态后必须自动执行暂存消息，不能再转成“待确认消息”要求用户回复“确认”。
- 实现约束：暂存项不能只保存文本，必须保留原消息的账号、会话、回复通道和客户端标识对应的延迟执行动作。
- 用户纠正：二次确认增加了无意义操作，任务队列应默认连续执行；上一任务报错也不能阻塞下一条，只有用户显式消费或撤回才停止续跑。

## 2026-07-10 Codex App 跨进程任务镜像（历史，当前优先使用 Desktop IPC 或共享 daemon）
...
## 2026-04-28 Codex 重启恢复（历史 owner 模型，当前使用 route binding）
...
## 2026-04-28 Codex 同会话并发（历史，当前使用 thread writer lease）

- 当前规则：同一 thread 同时只允许一个新 turn writer lease；已有活动 turn 时，其他已绑定前端的普通输入直接 steer 到该 turn，不创建第二个 lease。不同 thread 可以独立执行。
- 当时规则：同一微信用户、同一 Codex Agent、同一 workspace 不能并发进入 Codex turn；串行化边界包住进度会话、Agent 调用和最终回复。
...
## 2026-04-28 Codex 引导对话（历史，当前活动 turn 输入直接 steer）

- 当前规则：Codex 活动 turn 的普通输入直接发送并在飞书原生 stream 下创建接力卡；`/guide` 只处理确实存在的兼容暂存项，`/cancel` 只撤回暂存项。
- 当时规则：Codex 执行中收到第二条普通消息时先暂存并让用户明确选择 `/guide` 或 `/cancel`。
...
## 2026-07-25 飞书消息资源权限与事件重投

- 触发条件：机器人能收到图片消息事件，但 `GetMessageResource` 返回 `99991672`，同一图片反复失败并出现飞书回复子会话。
- 规则：入站事件权限与读取消息资源权限是两套能力；下载图片或文件还需 `im:message:readonly`、`im:message.history:readonly` 或 `im:message` 中至少一个。结构化权限错误对同一事件是终态，必须完成去重，不能靠飞书重投恢复。
- 反例：把资源 API 权限码转换成普通字符串错误并释放去重预约，同时回复原图片发送失败提示，会造成重复下载、重复提示和客户端子会话。
- 正确做法：资源下载错误保留飞书错误码；权限失败向当前主聊天发送具体权限引导，即使提示投递失败也收敛该事件。权限生效后由用户发送新消息触发新的资源下载。
- 来源：2026-07-25 `GetMessageResource` 返回 `99991672`，应用已有入站与资源权限，但缺少消息只读权限。
...
## 2026-07-07 飞书任务流里的会话导航卡片（历史，当前允许只读导航与显式切换）

- 触发条件：同一个飞书会话里 Codex 任务运行中，用户又发送 `/cx ls`、`/cx cd` 或点击会话导航按钮。
- 当前规则：只读导航和显式会话切换不再被 active task 一律阻断；切走后旧任务停止向当前窗口刷屏，真实切回仍运行的任务时在消息底部重建权威进度卡。独立结果消息负责终态通知，不能依赖阻止导航来维持视觉顺序。
- 当时规则：会生成飞书会话导航卡片的命令不能插入正在运行的任务流；任务运行中只回文本提示。
...
- 正确做法：最终结果独立发送时，进度流只更新为状态完成并保留已有结构化过程；没有结构化进度时只显示“已完成”状态，不合成缺少进度的解释，也不重复写入最终回答或“正在发送结果”类正文。失败和停止遵循同一状态收敛规则，必要信息由独立结果消息承载。
...
## 2026-07-08 普通最终回复不要按文本形态自动变成交互卡

- 触发条件：Agent 最终回复里包含“请选择”“1. ...”这类自然语言选项列表。
- 规则：不能仅凭“请选择”或编号列表等文本形态生成按钮交互卡；飞书 stream 的最终回复可以按明确的平台能力发送静态 Markdown 结果卡，但必须保持 Agent 原文语义，不能伪装成审批或系统选择。
...
- 规则：任务卡片实时状态只能展示结构化进度、Agent 计划/工具状态，或可证明为中间说明的用户可见消息；Codex commentary 从首条到末条进入累计时间线，Claude message 继续使用独立“当前说明”。协议未提供 phase 时必须延迟一条判定，不能把正常完成前仍待判定的最终回答或正文 delta 当作进度回填。
- 反例：把 `item/agentMessage/delta` 直接写入 progress，最终回复的 `## 交付说明` 或列表行会覆盖卡片状态，看起来像普通文本被渲染进卡片。
- 正确做法：Codex App 把 `turn/plan/updated` 与文件、MCP、动态工具生命周期转换为不含原始输出和参数的安全摘要；无信息量的 `commandExecution` 生命周期不投影到进度卡，只作为不含正文的继续执行信号，真正的命令审批继续独立展示。`phase=commentary` 立即以完整正文和稳定 ID 累计；phase 为空时，后续活动只把上一条消息确认为 commentary，正常终态前最后一条仍只用于最终消息发送。时间线默认不设条数上限并按卡片容量自动续卡。
- 来源：2026-07-08 用户指出“又把普通文本回复渲染成卡片了”和“任务卡片实时状态看着不行”。

## 2026-08-07 飞书任务卡等待态只保留单一提示

- 触发条件：飞书任务已经创建 stream 卡，但 Agent 尚未产生 commentary/message，或任务仍在继续生成下一段用户可见回复。
- 规则：首条有效非命令进度前，正文只显示 `思考中.....`；Codex commentary、Claude message、计划、文件或工具摘要任一到达后，就把累计回复与安全结构化进度写入卡片，并把该提示只保留一次在正文底部。定时心跳不能写入“等待 Agent”“连接正常”等替代文案，活跃任务卡也不能在顶部重复渲染同一状态。
- 反例：任务刚开始就展示文件、工具或阶段摘要，或者每隔一段时间把正文改成“仍在执行中”，用户会误以为 Agent 已经回复，也会让真正回复被无用状态淹没。
- 正确做法：reducer 可以先保存安全结构化事件；首条有正文的 commentary/message/plan/file/tool 即可触发网络更新，command/thought/status/approval 不能解除等待态。活动恢复引用和终态快照必须移除 `思考中.....`，完成、失败或停止只保留终态、过程正文与审批记录。
- 来源：2026-08-07 用户真机发现长任务只有文件事件时会一直显示“思考中.....”，确认改为首条有效非命令进度触发。

## 2026-07-08 飞书 admin_users 身份（历史，已由账号级 allowed_users 取代）

- 触发条件：判断飞书用户是否可执行 `/update`、`/restart`、`/feishu users` 等管理命令。
- 当时规则：飞书 `admin_users` 只允许使用 `union_id`；不保留 `open_id` 或 `user_id` 兼容。
- 反例：把某个机器人里的 `ou_...` 写进 `admin_users`，换另一个机器人后同一个人不再是管理员，或误以为多应用身份通用。
- 当时做法：飞书管理命令只读取 `feishu_union_id` 或 `on_` 形态的 union_id 别名，并通过额外管理员名单二次提权。
- 当前规则：`admin_users` 已完全退出运行时授权；当前机器人 `allowed_users` 中的身份具有相同管理能力，能力由 Registry 按平台、机器人账号和真实入站身份签发，不能跨机器人复用 `open_id`。

## 2026-07-08 飞书用户列表友好显示（历史角色文案）
...
## 2026-07-08 飞书授权码流程（历史命令示例，当前无 --admin）
...
- 当前做法：拒绝提示显示 `授权码: xxxxxx`；本地执行 `weclaw feishu users approve-code <code> [--bot <名称|app_id>]`，或由当前机器人已授权账号执行 `/feishu users approve-code <code>`；姓名只作为展示字段，不参与权限匹配。

## 2026-07-08 飞书首次管理员初始化（历史，admin_users 已废弃）

- 当时触发条件：`admin_users` 为空时，需要把第一个飞书管理员加入远程管理白名单。
- 当时规则：首次管理员初始化不能依赖飞书内管理命令，因为该命令本身需要管理员权限；必须提供本地 CLI 写配置路径。
- 反例：只支持飞书聊天命令写 `admin_users`，导致新安装环境只能手动编辑 `config.json`。
- 当前做法：本地 `weclaw feishu users approve <身份> [--bot <名称|app_id>]` 把首个可信身份写入目标机器人的 `allowed_users`；多机器人时必须显式指定 `--bot`，不再生成或维护顶层管理员名单。
...
- 当前做法：每条身份记录显示 `weclaw feishu users approve <id> --bot <名称|app_id>`；已配置 bot 显示为 `展示名 (name, app_id)`，单机器人时本地命令可省略 `--bot`。
- 来源：2026-07-08 用户指出 pending 输出缺少授权提示且机器人显示 id 没法区分。

## 2026-07-08 飞书任务卡片进度替换（历史实现已收敛为稳定组件更新）

- 触发条件：任务卡片实时进度展示“进展：...”状态。
- 当前规则：任务卡片进度是 reducer 的结构化快照，不是增量正文；飞书只保留一个稳定正文组件，默认实时写最近 5 条语义合并进度，展开后改写完整分段时间线，收起后恢复最新 5 条。结构变化、审批或终态才按 registry 的同一预览、完整正文和展开状态重建卡片，不能重置用户选择。
- 当时规则：旧版任务卡用整卡 `UpdateCard` 替换主内容，不能调用会追加内容的 CardKit 流式正文接口。
- 反例：把相同的 `进展：Codex 正在执行命令并产生输出。` 多次传给 `StreamContent`，飞书卡片会把它拼成一长段重复文本。
- 正确做法：普通任务进度用 `StreamContent` 替换同一 `main_content` 的当前可见快照，按钮、审批和终态等结构变化才用 registry 快照执行 `UpdateCard`；完整卡片容量预检必须始终使用展开态正文，且所有路径保留审批记录和单调 sequence。
- 来源：2026-07-08 用户截图反馈“任务卡片的实时进度有问题”。

## 2026-07-09 Codex App 实时进度可读行（当前禁止展示命令生命周期）
...
- 当前做法：`commandExecution` 生命周期、命令文本、输出和 patch 都不进入进度卡；只展示安全的计划、文件、工具摘要和可证明为 commentary 的用户可见说明，同一稳定 ID 的运行/完成状态原位更新。下面的命令行预览方案只保留为当时故障背景，不得重新启用。
...
## 2026-07-10 飞书卡片权限身份一致性（历史角色模型，现使用 Registry capability）

- 当时触发条件：飞书管理员通过 `union_id/user_id` 别名命中 `admin_users`，再点击工作空间导航卡片。
- 当时规则：卡片列表渲染和按钮回调执行必须透传同一个管理员判断结果，不能在回调阶段只按 `open_id` 重新判断。
- 反例：卡片按管理员权限显示完整工作空间，点击时按普通用户权限重新过滤，导致原编号提示不存在。
- 当前做法：Registry 在真实入站边界按平台、机器人账号和命中身份签发不可伪造的授权能力；文本命令与卡片回调统一复用该能力，任何客户端自报的管理员字段都不能提权。
...
## 2026-07-12 飞书短任务进度卡收敛（历史，当前立即创建等待卡）

- 触发条件：飞书使用原生流式进度卡，任务在初始展示延迟内完成，最终回复又要求单独发送文本。
- 当前规则：原生 stream 任务立即创建进度卡，第一条有效非命令进度前只显示 `思考中.....`；没有结构化进度的短任务终态只保留完成、失败或停止状态，并独立发送最终结果。排队与接管仍必须继承当前账号已解析的进度配置。
- 当时规则：进度流只在进度达到展示时机时创建，未展示进度的短任务直接发送最终回复。
...
## 2026-07-12 Codex Desktop no-client 的非幂等 Handoff 恢复（历史控制模型）
...
## 2026-07-14 Codex 本地与远程控制权必须显式移交（历史，现为协作前端绑定）
...
## 2026-07-17 Codex remote owner 是飞书写入的唯一授权门禁（历史，现为 frontend binding）
...
- 触发条件：飞书执行 `/cx ls`，Codex App 中项目和会话数量较多，卡片需要等待数秒才出现。
- 规则：顶层列表只读取工作空间名称、路径和排序；只有用户进入某个工作空间后，才查询该空间的会话。
- 反例：为了构造仅含项目名称的卡片，先遍历全部项目，逐个启动 SQLite 查询并重复解析完整 `session_index.jsonl`，随后生成卡片时再重复一次。
- 正确做法：项目头与带会话的完整 group 使用不同读取路径；工作空间选择解析完目标后只加载目标项目会话。回归测试应断言顶层列表不会产生按项目数量增长的会话查询。
- 来源：2026-07-20 用户反馈 `/cx ls` 卡片显示前等待明显；本机 21 个项目实测单次聚合由约 1.157 秒降至约 113 毫秒。
...
## 2026-07-30 包装后的哨兵错误必须使用 errors.Is 判断

- 触发条件：底层文件或系统错误经过一层或多层 `%w` 包装后，上层仍需区分“资源不存在”等可恢复分支。
- 规则：对 `os.ErrNotExist` 等哨兵错误统一使用 `errors.Is`；不要在错误已被业务上下文包装后继续依赖只兼容有限历史错误形态的辅助判断。
- 反例：安全文件组件把 `lstat` 错误包装为目录校验错误后，上层继续调用 `os.IsNotExist`，导致尚未创建的账号目录被误报为启动失败。
- 正确做法：保留 `%w` 错误链，并在边界使用 `errors.Is(err, os.ErrNotExist)`；同时补充缺失目录的包级回归测试和真实调用方测试。
- 来源：2026-07-30 凭据文件加固后的全量测试暴露微信账号目录缺失兼容回归。

## 2026-08-01 默认自动行为不能写成必选操作

- 触发条件：消息已经按默认策略排队自动续跑，同时卡片提供引导、撤回或停止等可选覆盖操作。
- 规则：卡片必须先明确“无需操作”和默认结果，再把按钮描述为仅在改变默认处理方式时使用；不能用“请选择如何处理”暗示用户必须操作。
- 反例：正文虽提到任务结束后自动执行，末尾却写“请选择如何处理”，并只列出会改变状态的按钮，使用户误以为没有自动续跑选项。
- 正确做法：把默认自动执行作为主状态展示，将引导、撤回和停止放在次要的可选操作区；不要为默认行为增加会消耗卡片但不改变状态的伪操作按钮。
- 来源：2026-08-01 用户指出飞书暂存消息卡片把可选操作呈现成了必选项。

## 2026-08-01 区域产品线的 Agent 接入能力必须独立核验

- 触发条件：同一 AI 开发工具同时存在国际版与中国版，且不同版本使用独立官网、账号体系、CLI 二进制、认证变量或协议文档。
- 规则：每个区域版本必须作为独立产品核验；只有该版本的官方资料或实机握手能证明其外部 Agent 协议、启动命令和认证方式，不能把另一版本的能力直接套用。
