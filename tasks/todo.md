...
## OpenCode ACP 错误可观测性修复清单

### 目标

OpenCode ACP 返回 JSON-RPC error 时，WeClaw 应保留 `error.message` 和 `error.data`，不能被 stderr 里的残缺 JSON 片段覆盖成 `agent error: }`。

### 执行任务

- [x] 本机按 initialize → session/new → session/prompt 顺序验证 `opencode acp` 可创建会话并返回文本。
- [x] 新增 JSON-RPC error 结构化 data 展示测试。
- [x] 新增 stderr 单独 `}` 不应覆盖主错误的回归测试。
- [x] 调整 JSON-RPC error 格式化逻辑，保留 message/data，并去重拼接有效 stderr。
- [x] 运行定向测试、全量测试、静态检查和 diff 检查。
...
## Codex attach 立即进入可见端清单

### 目标

解决真机执行 `/cx attach` 后 Terminal 只显示 `weclaw companion ...`、看起来像空等的问题；Companion 与 WeClaw 后台握手成功后，应立即启动 Codex app-server 和可见 Codex CLI。

### 执行任务

- [x] P1 串行：补 Codex Companion 启动后无需等待首条微信任务也会打开可见端的回归测试。
- [x] P2 串行：在 Companion 握手成功后调用可选启动钩子。
- [x] P3 串行：让 Codex app runtime 实现启动钩子，并复用原有 `ensureStarted`。
- [x] P4 串行：运行定向测试、全量测试、静态检查、构建和 diff 检查。

### Review 小结

已完成 Codex attach 立即进入可见端修复。`RunCompanionClient` 在发送 hello 后会识别可选 `CompanionStarter` 并立即启动本地运行时；Codex app Companion 复用原有 `ensureStarted`，因此 `/cx attach` 打开的 Terminal 会主动启动 Codex app-server 和可见 Codex CLI，不再等到第一条普通微信任务。验证命令：`go test ./cmd ./agent -run 'TestCodexAppCompanionRuntimeStartOpensVisibleAttachWithoutTurn|TestCodexAppCompanionRuntimeStartsServerAttachAndRunsTurn|TestRunCompanionClientHandlesRequest|TestRunCompanionClientStartsRuntimeAfterHello' -count=1`、`go test -count=1 -timeout 60s ./...`、`go vet ./...`、`go build -o /tmp/weclaw-attach-eager .` 与 `git diff --check`，结果通过。

## Codex App 接手入口清单

### 目标

支持在微信侧把当前 Codex 工作空间打开到本地 Codex App，便于用户回到电脑后从 App 会话列表接手继续。

### 执行任务

- [x] P1 串行：补 `/cx app` 打开当前工作空间并回显 thread 的 Handler 测试。
- [x] P2 串行：补 `/cx attach app` 作为打开 Codex App 的别名测试。
- [x] P3 串行：新增可测试的 Codex App opener，并用当前 Agent command 执行 `codex app <workspace>`。
- [x] P4 串行：同步帮助文本和微信回复格式。
- [x] P5 串行：运行定向测试、全量测试、静态检查、构建和 diff 检查。

### Review 小结

已完成 Codex App 接手入口。微信发送 `/cx app` 或 `/cx attach app` 会使用当前 Codex Agent 的 command 打开 `codex app <当前工作空间>`；如果当前 Agent 能提供 threadID，回复会一并显示该 thread，方便在 App 中确认对应会话。验证命令：`go test ./messaging -run 'TestCodexAppCommandOpensCurrentWorkspaceWithThread|TestCodexAttachAppAliasOpensCodexApp|TestCodexAttachOpensVisibleCompanion|TestCodexDetachClosesVisibleCompanionOnly|TestCodexAttachRequiresVisibleCompanion|TestBuildHelpText|TestCommandRepliesUseBlankLinesForWeChat' -count=1`，结果通过；全量验证结果见本轮交付说明。

## Codex remote-first 接手语义清单

### 目标

Codex 普通微信任务默认走 app-server remote-first，不再依赖本地 Companion；需要电脑接手时，`/cx attach` 使用当前 thread 打开本地 Codex CLI/TUI 继续。

### 执行任务

- [x] P1 串行：补旧 Codex Companion 配置自动迁移为 app-server remote-first 的测试。
- [x] P2 串行：补 `/cx attach` 对 remote-first Codex thread 执行 CLI resume 的 Handler 测试。
- [x] P3 串行：补 remote-first 尚无 thread 时的明确提示测试。
- [x] P4 串行：实现 `NormalizeCodexRemoteFirst`，保留用户显式 `auto_launch: true` 的旧 Companion 配置不迁移。
- [x] P5 串行：新增可测试的 Codex CLI resume opener，默认在 macOS Terminal 中执行 `codex resume <thread> --cd <workspace>`。
- [x] P6 串行：运行定向测试、全量测试、静态检查、构建和 diff 检查。
...
避免 `/cx ls` 把本机 Codex 历史 session 中已经不存在的 `cwd` 当成可切换工作空间展示。

### 执行任务

- [x] 串行：新增缺失 `cwd` 不进入本机 Codex 会话扫描结果的回归测试。
- [x] 串行：扫描 `~/.codex/sessions` 时只保留当前仍存在的目录。
- [x] 串行：运行定向测试、全量测试、静态检查、构建和 diff 检查。
