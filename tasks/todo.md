...
- [x] Task 8：收口发布级复核发现的账号跨重启 fail-closed、保存补偿、secret 清理、ACP 进程回收、微信路径/登录单飞与发布 workflow 重复实现。

## 当前状态

发布级复核整改与自动化门禁已完成：账号切换未知终态可跨重启 fail-closed，在线保存具备跨索引与 Host 元数据补偿，旧 secret 清理可持久化重试，ACP 子进程统一回收，微信账号路径与 Web 登录单飞边界已收敛，稳定版 workflow 已委托权威发布脚本。
