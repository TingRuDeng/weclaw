...
- `agent/` 内包含 ACP、CLI、HTTP、Companion 等 Agent runtime；verified official standalone daemon 是 Codex 的默认共享 Host authority。新版 Codex App 的 Code Mode 可为已打开 thread 启动 App 自己的受锁 Host：只有 daemon 明确返回 active writer，且安全 Desktop IPC 确认同一 thread 的 active turn ID 时，WeClaw 才让该 thread 改用 follower steer；其他 thread 仍使用 daemon，WeClaw 不自行启动第二 Host。`codex_multi_frontend: true` 会把默认 Host 模式固定为 `daemon`，在平台启动前要求 standalone、App 复用与 Host 身份验证通过。standalone 不可用时的 App 私有 Host 或 WeClaw-managed Host 只是兼容路径。显式 `managed` 下消息平台与受控 `weclaw codex cli` 复用同一 WeClaw-managed Host。活动 turn 输入直接 steer，未发送的 Desktop/CLI follow-up 仍是客户端草稿。
...
- 原生 Codex `auto`/`daemon` 默认让后续启动的 macOS Codex App 继承受保护的 local-daemon 环境；设置前必须验证 daemon 与 App 推导出的 `CODEX_HOME` control socket 一致。App 自带的 Code Mode helper 可能持有单个 thread writer，不视为未标记私有 Host；未带标记的私有 app-server 仍失败关闭并要求用户重启 App。WeClaw 启动 daemon 时使用私有 `umask 0077`，外部 PID 记录仍须通过权限和身份验证。
