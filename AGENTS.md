...
- `scripts/release.sh` 只为 GitHub 构建和发布 `darwin/arm64` 正式资产及其原始摘要，Gitee 镜像同一资产的压缩表示和摘要；CI 仍可交叉构建其他目标做兼容性检查，但不得把它们上传到正式或预发布 Release。发布会运行测试、race、vet 和 `git diff --check`，本地发布通过 `WECLAW_GOCACHE`、调用方 `GOCACHE` 或平台默认值统一复用单一持久化 Go 缓存。
