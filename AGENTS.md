...
- `agent/` 内包含 ACP、CLI、HTTP、Companion 等 Agent runtime；包含 Codex App 的完整多前端共享只在已验证 official standalone daemon 上成立，它是唯一 Host authority，Codex App 只是连接该 daemon 的 frontend/history view。`codex_multi_frontend: true` 会把有效 Host 模式固定为 `daemon`，在平台启动前要求 standalone 已安装，并同步完成 daemon、App 复用和 Host 身份验证；失败时禁止 managed 回退和平台先上线。省略时保留旧 `auto` 兼容行为，显式 `false` 不得被规范化重新打开 App daemon 复用。macOS `auto` 在固定 socket 上已有已验证 daemon 或 standalone 可用时会在构造期固定为 `daemon`，不因 App 可见而切换 Desktop authority；standalone 不可用时的 App 私有 Host 或 WeClaw-managed Host 只是兼容路径，不承诺 App 共享。显式 `managed` 可使用配置中的 npm Codex 命令；消息平台与受控 `weclaw codex cli` 复用同一 WeClaw-managed Host，但 CLI 必须在服务运行时接入，不能自行启动第二个 Host。由协调停止确认旧 daemon 已停止后，重启事务只允许在 Desktop 缺席且当前 Host 身份与多 Host 预检通过时迁移到显式 managed；其他非 daemon 拓扑变化继续失败关闭。显式 `daemon` 可使用 Desktop IPC 协调，但不允许选择 App Host。活动 turn 输入直接 steer，未发送的 Desktop/CLI follow-up 仍是客户端草稿；Codex Companion 和旧 `codex exec` 第二 writer 已停用。
- shared managed Host、official daemon、受控 `weclaw codex cli` 与协调停止在启动、接管或变更 Host 前执行只读多 Host 预检：普通模式检查当前有效 UID，`run_as_user` 还检查目标 UID 及只用于 sudo wrapper 的 root UID；macOS/Linux 从系统接口读取候选进程的原始 argv，按 PGID 聚合 Node/native 进程，仅放行 metadata 或 lifecycle PID 证明的权威组。额外 Host、进程表或原始参数不可读、身份不确定时失败关闭；`--remote`、帮助、daemon/proxy/schema generation 等 tooling 不算 Host。错误只显示脱敏 PGID/PID，普通预检不停止进程。显式 `--force` 是唯一例外：它先请求 Codex App 退出，再对当前用户的实际 Codex `app-server` 进程组做实时 UID、PGID、启动时间、可执行文件和 argv 指纹复核后强制终止，不仅凭旧 PID 或名称杀进程；管理身份未知时还要求 app-server 自己领衔独立进程组，拒绝连带结束共享 PGID 的 shell 或其他程序。该检查是时点门禁，不是持续全局锁；Desktop 私有 Host 兼容路径不因此获得受管身份。
...
- 原生 Codex `auto`/`daemon` 默认让后续启动的 macOS Codex App 通过受保护的 launchd 环境复用同一官方 daemon；设置前必须验证 daemon 与 App 推导出的 `CODEX_HOME` control socket 完全一致。WeClaw 调用官方 daemon `start` 时固定使用私有 `umask 0077`，不得继承登录 shell 的宽松 umask；外部既有 daemon 的 PID 记录仍必须通过原有权限与身份校验，不能自动修权后接管。已运行 App 若仍带私有 app-server，只能失败关闭并要求用户重启 App，禁止自动退出或按进程名清理；独立 `CODEX_SQLITE_HOME` 表示 App 接入后会看到 daemon 的目录，不得据此迁移或删除旧目录。
