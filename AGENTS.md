...
- `scripts/release.sh` 为 GitHub 构建和发布 `darwin/arm64`、`linux/arm64`、`linux/amd64` 正式资产及原始摘要，Gitee 只镜像 `darwin/arm64` 的压缩表示和同一摘要；CI 额外交叉构建 `darwin/amd64` 做兼容性检查，但不得上传该资产。发布会运行测试、race、vet 和 `git diff --check`，本地发布通过 `WECLAW_GOCACHE`、调用方 `GOCACHE` 或平台默认值统一复用单一持久化 Go 缓存。
