...
func (h *Handler) codexAppWorkspaces() ([]codexAppWorkspace, bool, error) {
	h.mu.RLock()
	dir := h.codexLocalSessionDir
	h.mu.RUnlock()
...
func (h *Handler) codexAppWorkspaceThreads(workspaceRoot string) ([]codexWorkspaceView, bool, error) {
	h.mu.RLock()
	dir := h.codexLocalSessionDir
	h.mu.RUnlock()
