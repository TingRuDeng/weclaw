...
func TestFeishuCodexWorkspaceChoicesUseStablePagination(t *testing.T) {
	...
	if len(choices) != 3 || choices[0].Label != "8. workspace-07" || choices[1].Label != "9. workspace-08" ||
...
func TestFeishuCodexCxLsSendsWorkspaceChoices(t *testing.T) {
	...
	if !isTestFeishuWorkspaceChoice(choices[0].ID, "/cx") || choices[0].Label != "1. alpha" {
...
func TestFeishuCodexWorkspaceChoiceSendsSessionChoices(t *testing.T) {
	...
	choices := reply.Choices[0].Choices
	if len(choices) != 3 || choices[0].ID != "/cx switch thread-a" || choices[0].Label != "1. 会话 A" || choices[1].Label != "2. 会话 B" {
...
func TestFeishuCodexSessionChoicesCanReturnToWorkspaceList(t *testing.T) {
	...
	if len(workspaceChoices) != 2 || !isTestFeishuWorkspaceChoice(workspaceChoices[0].ID, "/cx") || workspaceChoices[0].Label != "1. alpha" {
