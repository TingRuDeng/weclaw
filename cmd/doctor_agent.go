...
func probeClaudeACP(ctx context.Context, name string, agentCfg config.AgentConfig, factory doctorACPFactory) error {
	client := factory(name, agentCfg)
	if client == nil {
		return fmt.Errorf("ACP 探针创建失败")
	}
	defer client.Stop()
	probeCtx, cancel := context.WithTimeout(ctx, doctorACPProbeTimeout)
	defer cancel()
...
func defaultClaudeACPProbe(ctx context.Context, name string, agentCfg config.AgentConfig) error {
	return probeClaudeACP(ctx, name, agentCfg, func(agentName string, cfg config.AgentConfig) doctorACPClient {
