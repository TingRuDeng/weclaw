...
func newWechatUsersApproveCodeCmd(commandPath string) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args: func(cmd *cobra.Command, args []string) error {
			...
			return fmt.Errorf("用法: %s approve-code <授权码>", commandPath)
		},
		RunE: func(cmd *cobra.Command, args []string) error {
			result, err := messaging.ApproveAccessCode(messaging.AccessCodeApprovalRequest{Code: args[0]})
			if err != nil {
				return err
			}
...
// runUsersList 输出微信 allowed_users，便于服务器维护者确认当前授权状态。
