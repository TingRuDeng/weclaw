...
- 飞书推荐使用 7.22+ 悬浮菜单，菜单项通过“发送文字消息”触发命令，只保留 `/help`、状态/任务控制、Codex/Claude 列表与新建、模型/推理/确认模式等高频入口；可切换菜单受 3 个主菜单限制时移除“设置”主菜单。`/guide` 和 `/cancel` 不进入常驻菜单：运行中暂存第二条消息后立即显示上下文操作卡，Codex 提供引导、撤回、停止，Claude 只提供撤回、停止；按钮必须绑定机器人账号、真实操作者、route、活动 task ID、暂存 revision 和正文指纹，旧卡不得操作后来任务或同文替换消息，处理结果原地更新该卡。`/help` 仍按 Agent 分类提供命令兜底，管理员分类只对管理员显示；已停用的 `/cc owner`、`/cc cli` 不得继续出现在帮助卡中。普通计划确认仍回复“确认”，暂存消息未被 `/guide`、`/cancel` 或 `/stop` 消费时会在上一任务结束后自动执行。
