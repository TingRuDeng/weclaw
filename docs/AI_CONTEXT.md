...
- `messaging/codex_session_acquire.go:acquireCodexSessionWithBindingLocked` 先提交 route 选择和唯一 remote owner，再持久化当前窗口 Agent，最后同步 runtime handoff 和外部活动任务 observer。Agent 选择落盘失败时只在 after-image 仍匹配时回滚 store。显式 remote 接管遇到 Desktop timeout、断线或旧 conflict 时会在 checkpoint 校验后恢复 WeClaw app-server；真正的 runtime/observer 失败仍保留 owner 并报告通道不可用。
