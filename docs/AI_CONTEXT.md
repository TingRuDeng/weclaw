...
- Codex 运行拓扑是单一 app-server、多前端客户端。`codex_host_mode: auto` 只在 `CODEX_HOME` 已有官方 control socket 或可执行 standalone Codex 时选择 `agent/codex_daemon_host.go`，通过 standalone 自身执行 `codex app-server daemon start/version/stop` 并验证 backend、socket、PID 记录、启动时间和受管二进制；任一校验失败都不得回退启动第二个 Host。未安装 standalone 时使用 `agent/codex_app_server_host.go` 的 WeClaw-managed 兼容路径。两种路径都按上游标准 HTTP Upgrade 建立 WebSocket-over-UDS，禁止把 socket 当裸 JSONL `net.Conn`。兼容路径超过 `sockaddr_un` 限制时稳定哈希到真实系统临时目录下的用户私有 `weclaw-<uid>` 目录；官方 daemon 固定使用 `CODEX_HOME/app-server-control/app-server-control.sock`，路径过长必须失败。host 生命周期独立于单次前端请求，普通客户端断开和恢复不得终止其他前端正在使用的 host。
...
- `/cx` 是 Codex 会话命令专用命名空间，未知子命令统一返回 `/cx help`；Codex 消息使用 `/codex <内容>` 或 `@cx <内容>`。`/cx owner` 及其旧参数已删除；`/cx status` 正常态只紧凑展示工作空间短名、会话、任务、账号标签和运行状态，当前 thread 为 Fast 时额外显示一行速度；不请求额度，异常态继续显示查询繁忙、运行不可用或写入冲突。完整路径、账号诊断和额度分别由 `/cx pwd`、`/cx account status`、`/cx quota` 展示。`/cx app`、`/cx cli`、`/cx attach`、`/cx detach` 统一拒绝，旧 Codex Companion 配置无论 `auto_launch` 是否为 true 都迁移到 ACP 共享 host，`weclaw companion --agent codex` 必须在等待 endpoint 前立即拒绝，禁止重新引入第二 app-server。`/model`、`/reasoning`、`/fast` 在窗口已绑定 Codex thread 时必须调用 app-server `thread/settings/update`，从同一 thread 的下一轮任务生效；无绑定时才修改显式新建 thread 的进程内默认值；`/cx model status` 只展示新建 thread 默认值，文案必须与当前 thread 配置明确区分。`/fast` 从 `model/list.serviceTiers`（兼容 `additionalSpeedTiers`）确认当前模型是否提供 Fast，规范请求使用 `priority`，关闭时发送显式 `serviceTier: null`，未知能力不得静默降级。新 thread 默认值只进入 `thread/start`，`thread/resume` 和后续 `turn/start` 禁止重复注入共享默认值；当前 thread 配置从 `thread/start`/`thread/resume` 响应及 `thread/settings/updated` 通知缓存，必须区分未知与明确标准，旧 sequence 不能覆盖新配置，`thread/read` 不包含该配置。
