...
- `/cx owner` 仅保留为只读兼容状态；`/cx owner remote|desktop` 不再移交 writer。`/cx app`、`/cx cli`、`/cx attach`、`/cx detach` 统一拒绝，旧 Codex Companion 配置无论 `auto_launch` 是否为 true 都迁移到 ACP 共享 host，禁止重新引入第二 app-server。
