...
- 飞书任务卡成功写入完成终态后不再补发成功消息，也不在卡片正文重复显示“已完成”；成功状态由绿色标题和最终内容表达，无最终内容时只保留绿色标题。失败或停止只有在卡片终态写入成功后才发送简短通知。任务终态先把可恢复文本草稿写入 `~/.weclaw/state/terminal-outbox.json`，再冻结任务卡并以 CardKit checkpoint 原子替换草稿，最后触发网络调用；进程在冻结与替换之间退出时，重启后仍会续投恢复文本。文件固定为 `0600`、同目录原子 rename 并 fsync。CardKit checkpoint 固定 UUID 与单调 sequence，飞书文本固定消息 UUID，微信文本固定分片 client_id；语义是平台幂等辅助下的 at-least-once，不承诺 exactly-once。附件与远程图片不进入 v1 outbox，仍按原有校验和 best-effort 路径发送。运维只通过脱敏的 `weclaw outbox status` 查询积压；`weclaw outbox redrive [entry-id]` 仅在服务运行时通过真实 loopback 和既有 API token 提前唤醒重试，不重置 attempts、不修改正文，也不允许离线改文件。`weclaw doctor` 必须把文件损坏、积压和容量耗尽显式报告。
