...
- 飞书多项目入口通过 `platforms.feishu.bots[]` 配置多个机器人；每个 bot 的凭证按 `weclaw feishu login --name <bot>` 或 `weclaw feishu bootstrap --name <bot>` 保存，`app_secret` 不得写入 `config.json`。飞书与微信账号凭据、微信主动发送所需的 `context_token` 统一通过 `internal/securefile` 读写：目录固定 `0700`、文件固定 `0600`、原子替换，拒绝符号链接、异常 owner 和宽权限文件；损坏或不可读的 token 状态必须阻止对应 adapter 启动，不能静默当成空状态覆盖。共享同一 `WECLAW_HOME` 的微信凭据与 `context_token` 读改写事务统一通过 `internal/accountstore` 的进程内门禁和 Unix advisory lock 串行化；固定的 `accounts/.write.lock` 必须持久保留，不能删除或原子替换。
