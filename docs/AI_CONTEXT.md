...
- `messaging/codex_session_acquire.go:acquireCodexSessionWithBindingLocked` 先提交 route 选择和唯一 remote owner，再持久化当前窗口 Agent，最后同步 runtime handoff 和外部活动任务 observer。Agent 选择落盘失败时只在 after-image 仍匹配时回滚 store；runtime 或 observer 失败不撤销窗口所有权，而是把目标运行绑定标记为 conflict 并禁止普通写入。
