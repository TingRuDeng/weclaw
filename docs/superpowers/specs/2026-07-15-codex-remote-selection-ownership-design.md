...
- 可确认的运行时移交、状态持久化或观察启动失败时，恢复原选择和原控制权；补偿结果无法确认时保持 fail-closed。
...
7. 无法确认唯一 writer 时进入 fail-closed，禁止新写入。
...
超时后不得重发可能产生副作用的 IPC 请求。系统先使用旧持久化意图重新探测和校准内存 registry；如果仍无法确认唯一 writer：

- thread 标记为冲突或所有权未知。
- 新任务、`/guide` 和 `/stop` 均拒绝执行。
- 用户收到“移交结果未确认”的明确回复。
- 后续 `/cx owner`、`/cx status` 或重新选择会再次探测，但不猜测成功。

### 8.3 进程崩溃窗口

外部运行时变更与本地状态文件无法组成真正的跨进程 ACID 事务。实现采用 saga：先完成实际运行时移交，再原子持久化，失败时逆序补偿。

如果进程在两者之间被强制终止，重启后以持久化控制意图为期望状态，重新探测实际 runtime。无法确认时保持 fail-closed，不根据进程存在或超时自行决定控制方。
