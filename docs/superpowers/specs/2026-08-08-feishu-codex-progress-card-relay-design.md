...
接力功能已随正式版本发布；2026-09-16 起，当前源码改为默认展示完整进度，本文下方的最近 5 条预览与折叠交互保留为历史设计。

本文定义飞书在 Codex 活动 turn 上发送引导消息时的任务卡接力语义，并记录原生进度卡展示契约的演进。当前任务卡完整展示 Codex 用户可见说明，取消五条裁剪与新卡展开／收起按钮，超长内容继续按容量自动续卡。当前产品事实仍以 `docs/AI_CONTEXT.md`、源码和测试为准；完整展示尚未经过真实飞书桌面端与移动端验收。
...
- 飞书任务卡需要在同一 Markdown 正文中兼顾可扫描的最近进度与可追溯的完整时间线，不能同时渲染两份重复正文，也不能由普通流式更新重置用户的展开状态。
...
- 任务执行中的最新卡默认显示最近 5 条，并允许用户在同一正文区域展开完整进度或收起回预览；已转移、已完成、已停止和失败卡自动收起。
...
| --- | --- | --- | --- | --- |
| `thinking` / `streaming` | 是 | 否，默认预览 | 接收 | 当前任务仍在执行 |
...
3. 只有 Codex 明确接受引导后，WeClaw 才在该消息下创建默认显示最近 5 条的新任务卡。
4. 新卡以当前 reducer 快照初始化，同时保存最近 5 条预览、完整结构化时间线、展开状态和思考中标记。
...
飞书任务卡继续使用 Card JSON 2.0 和 CardKit 组件更新。早期设计采用 `collapsible_panel`，当前实现改为服务端持久化的展开状态与互斥按钮，以便在同一个稳定 `main_content` 中准确切换预览和完整正文。参考：[CardKit 流式更新](https://open.feishu.cn/document/cardkit-v1/streaming-updates-openapi-overview?lang=zh-CN)。

任务卡拆成稳定区域：

1. 卡片 header：Agent、工作空间和任务标题。
2. 审批记录：独立于进度正文，位于正文之前。
3. 唯一 `main_content`：收起态显示当前分段最近 5 条，展开态显示本分段完整时间线。
4. 互斥控制按钮：收起态显示“展开完整进度”，展开态显示“收起完整进度”，始终位于卡片底部。

普通进度事件只通过 CardKit 内容更新接口替换当前可见的 `main_content`；不再因任务卡被 registry 跟踪而每次全量重建整卡。审批、按钮和终态等结构变化可以全量更新，但必须从 registry 的同一预览、完整正文和展开状态快照重建，不能丢失进度或用户选择。

展示规则：

- 新建活动卡设置 `expanded=false`，正文显示当前分段最近 5 条语义合并进度；不足 5 条时全部显示。
- 用户手动折叠或展开活动卡后，普通流式内容更新不得改变该客户端状态。
- 收起态把“展开完整进度”按钮置于预览正文之后；展开态在完整时间线末尾显示“收起完整进度”按钮，其回调只更新原任务卡，不进入 Agent 命令路由。
- `Supersede` 和终态更新是明确的结构更新，统一写入 `expanded=false`。
- 预览和完整时间线不得同时作为两个用户可见正文区域；完整时间线在收起后仍保留在 registry 和 durable reference 中。
- 现有保守 Card JSON 字节上限和超长续卡逻辑继续生效，初始创建和后续更新都必须按展开态完整 JSON 预检。

消息层不得通过解析已渲染 Markdown 猜测预览。`progressSession` 已持有结构化 `progressCardSnapshot`，通过结构化流式展示接口同时传递 `Summary`、最近 5 条 `Preview` 与完整 `Details`；不支持该接口的平台继续接收原有单字符串 `Stream.Update`。
...
- `feishu/card.go`：构建唯一进度正文、审批区域和互斥展开/收起按钮，并按状态选择预览或完整正文。
...
- 卡片 JSON：活动卡默认 `expanded=false` 并显示最近 5 条预览；已转移、完成、停止和失败卡仍为 `expanded=false`；展开和收起都只保留一个正文区域及一个底部互斥按钮。
- 组件更新：普通进度根据展开状态只更新同一正文元素，不全量重建卡片；结构终态执行完整更新并收起。
...
- 活动卡默认显示最近 5 条，可以展开完整进度并收起回最新 5 条。
- 收起或展开后，持续流式更新写入当前可见版本，不会自动切换或重置状态。
