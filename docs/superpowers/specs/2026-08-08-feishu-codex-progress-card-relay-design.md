...
接力功能已随正式版本发布；最近 5 条单区交互已在当前源码实现，待发布及真实飞书端验收。

本文定义飞书在 Codex 活动 turn 上发送引导消息时的任务卡接力语义，并记录原生进度卡展示契约的演进。接力功能已经发布；任务卡已改为默认显示最近 5 条、按按钮在同一正文区域切换完整进度，当前产品事实仍以 `docs/AI_CONTEXT.md`、源码和测试为准。新交互在完成飞书桌面端与移动端验收前，不能描述为已通过端侧验收。
