...
已实施并随当前正式版本发布，待真实飞书端验收。

本文定义飞书在 Codex 活动 turn 上发送引导消息时的任务卡接力语义，以及原生进度卡的折叠展示契约。实现、自动化验证和正式发布已经完成，当前产品事实仍以 `docs/AI_CONTEXT.md`、源码和测试为准；在完成飞书桌面端与移动端验收前，不能把这些行为描述为已通过端侧验收。
...
审批记录保持独立区域并位于折叠面板之前，使面板成为卡片最后一个内容区域。普通进度事件通过 CardKit 内容更新接口分别更新“当前摘要”和“完整进度”稳定元素；不再因任务卡被 registry 跟踪而每次全量重建整卡。审批结构变化可以全量更新，但必须从 registry 的同一快照重建，不能丢失进度、状态或面板默认值。

展示规则：

- 新建活动卡设置 `expanded=true`。
- 用户手动折叠或展开活动卡后，普通流式内容更新不得改变该客户端状态。
- 收起态把“展开完整进度”原生面板入口置于卡片可见内容末尾；展开态在完整时间线末尾增加“收起完整进度”按钮，其回调只更新原任务卡，不进入 Agent 命令路由。
...
- 卡片 JSON：活动卡 `expanded=true`；已转移、完成、停止和失败卡 `expanded=false`；摘要与审批位于折叠区外且面板为最后一个区域，完整进度及底部收起按钮位于面板内。
