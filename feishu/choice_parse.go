...
func parseCardAction(event *callback.CardActionTriggerEvent) (parsedCardAction, bool) {
	...
	parsed := parsedCardAction{
		...
		AgentName: firstNonEmpty(
			callbackValueString(value, platform.ChoiceMetadataAgentName),
			callbackValueString(value, modelSettingAgentKey),
		),
		CodexThreadID:      callbackValueString(value, modelSettingCodexThreadKey),
		ClaudeSessionID:    callbackValueString(value, modelSettingClaudeSessionKey),
		...
		TaskControlToken:   callbackValueString(value, platform.ChoiceMetadataTaskControlToken),
