...
| `/ps`                   | 查看自己运行中的任务     |
| `/stop`                 | 停止当前运行的任务       |
| `/update` / `/upgrade`  | 管理员远程更新 WeClaw（需配置 `admin_users`） |
...
| `/cx new` | 新建当前工作空间会话 |
| `/cx new-thread` | 在飞书单聊中开启一个回复串子会话 |
...
飞书群聊的回复串 / 话题，以及飞书单聊中通过 `/cx new-thread` 创建的回复串，都会作为独立的 WeClaw 子会话。每个子会话独立保存 Codex workspace 和 thread；在一个子会话里 `/cx cd` 或 `/cx switch` 不会切走其他子会话。
...
远程管理命令由 WeClaw 自身执行，不会进入 Codex / Claude 等 Agent：`/update` 和 `/upgrade` 调用当前 WeClaw 二进制的自更新逻辑；`/restart` 会先回复，再异步触发 `weclaw restart`，避免消息尚未发出时服务退出。
