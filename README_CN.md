...
其他：`/cx whoami`、`/cx ls`、`/cx ..`、`/cx cd <工作空间|..>`、`/cx pwd`、`/cx status`、`/cx quota`、`/cx model status|ls`、`/cx clean`。其中 `/cx model status` 查看后续新建 Codex 会话的默认配置；当前绑定会话请用 `/model`、`/reasoning`。
