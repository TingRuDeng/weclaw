...
通过微信和飞书远程接管本机 Codex、Claude：复用真实工作空间与会话上下文，实时回传进度、审批和结果，并在 Codex Desktop 与远程窗口之间显式移交控制权。
...
- **控制权明确**：Codex 会话通过 `/cx owner` 在 Desktop 与远程窗口之间显式移交，避免双写污染。
...
没有有效会话绑定时，普通消息只会提示选择会话或发送 `/cx new`，不会隐式创建新会话。

### 接管并归还 Codex Desktop 会话

```text
/cx ls                 # 选择本机已有 workspace 和 thread
/cx owner              # 查看当前控制方、运行位置和任务状态
/cx owner remote       # 将控制权交给当前微信或飞书窗口
/cx owner desktop      # 空闲后归还给 Codex Desktop
```

控制权移交会先完成运行时探测和状态提交，再返回成功；远程任务执行中必须先等待完成或发送 `/stop`，不能中途归还 Desktop。
...
| `/new` | 为当前默认 Agent 明确新建会话 |
| `/model`、`/reasoning` | 查看或切换当前会话的模型和推理强度 |
| `/mode [default|yolo]` | 查看或切换当前用户的 Codex 审批方式 |
...
`/cx ls`、`/cx <编号|..>`、`/cx cd <工作空间|..>`、`/cx switch <会话>`、`/cx new`、`/cx pwd`、`/cx status`、`/cx owner [remote|desktop]`、`/cx quota`、`/cx model status|ls`、`/cx cli`、`/cx app`、`/cx clean`、`/cx detach`。
...
`/cc ls`、`/cc switch <编号|sessionId>`、`/cc new`、`/cc pwd`、`/cc status`、`/cc model status|ls`、`/cc cli`。
