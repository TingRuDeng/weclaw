...
| CLI  | 每条消息启动一个新进程。                                       | Codex (`codex exec`)                                    |
| HTTP | OpenAI 兼容的 Chat Completions API。                             | OpenClaw（HTTP 回退）                                   |
| Companion | WeClaw 后台接微信，本地终端保持可见 CLI 连接。              | OpenCode                                               |

Claude 远程后端只支持 ACP；原生 `claude` 仅作为空闲 session 的本地交接命令。Codex 默认使用 `app-server --listen stdio://` 的 remote-first 模式：微信侧独立使用当前 workspace/thread，本地需要接手时再通过 `/cx cli` 或 `/cx app` 打开本地入口。
...
| `/new`                  | 开始新对话（清除会话）   |
| `/model` / `/model <id>` | 查看 / 切换当前会话 Agent 的模型；Claude 已绑定 session 时立即生效，无绑定时需先选择或新建 session |
| `/reasoning` / `/reasoning <强度>` | 查看 / 切换当前会话 Agent 的推理强度；Claude 已绑定 session 时立即生效，无绑定时需先选择或新建 session |
...
Claude 使用 ACP `session/list`、`session/resume` 和 `session/new` 管理真实 Claude Code session。微信和飞书选择会话后会持续复用同一上下文，重启后在下一条消息前恢复绑定：

| 命令 | 说明 |
| ---- | ---- |
| `/cc ls` | 查看 Claude ACP 返回的工作空间和可恢复会话 |
| `/cc switch <编号|sessionId>` | 切换到指定 Claude session，编号来自 `/cc ls` |
| `/cc new` | 立即在当前工作空间创建并绑定新 session |
| `/cc pwd` | 查看当前 Claude 工作空间 |
| `/cc status` | 查看当前工作空间、session、恢复状态、模型和推理强度 |
| `/cc cli` | 当前任务空闲时，在本地 Terminal 中用 `local_command --resume` 接手 session |
| `/cc help` | 查看 Claude 会话命令 |

Claude ACP 任务会先创建进度卡再在后台执行；运行中最多暂存一条后续消息，任务成功或失败后都会自动续跑。`/cancel` 撤回暂存消息，`/stop` 停止当前任务；Claude ACP 不支持 `/guide`。

WeClaw 不扫描 `~/.claude` transcript。会话目录以 ACP 返回结果为唯一事实源；没有有效绑定时，普通消息只提示 `/cc ls` 或 `/cc new`，不会隐式新建会话。
...
> **注意：** 这些参数会跳过安全检查，请了解风险后再启用。Claude 远程后端不支持 CLI 模式；ACP Codex Agent 使用 `permission_level` 控制权限边界，无需通过 CLI 参数跳过审批。
