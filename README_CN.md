...
/cx status             # 查看共享 host、workspace 和 thread 状态
...
| `/cx owner` | 查看共享 app-server 兼容状态；不再切换独占 writer |
...
运行边界：`/cx status` 查看共享 host 和当前绑定；`/cx owner` 仅保留为只读兼容状态。

其他：`/cx ls`、`/cx ..`、`/cx cd <工作空间|..>`、`/cx pwd`、`/cx status`、`/cx quota`、`/cx model status|ls`、`/cx clean`。
